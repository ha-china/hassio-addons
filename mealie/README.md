...
我利用业余时间维护这个和其他 Home Assistant 插件：跟进上游更改、HA 更改以及在真实硬件上测试需要大量时间（和一些金钱）。我大约使用我 >110 个插件中的 5-10 个，因此我安装了测试机器（并购买了一些我自己不使用的测试服务，如 VPN）来调试和改进插件。
...
_感谢所有将我的仓库标记为星标的人！要标记它，请点击下面的图片，然后它将在右上角。谢谢！_
...
Mealie 是一个自托管食谱管理和餐食规划工具，具有 RestAPI 后端和基于 Vue 的响应式前端应用程序，为整个家庭提供愉悦的用户体验。
...
要配置 Gunicorn 的 `--forwarded-allow-ips` 设置以供受信任的反向代理，手动在您的插件配置中添加 `FORWARDED_ALLOW_IPS` 条目（逗号分隔的 IP）。它是可选的，并且默认选项卡中不显示。例如：`FORWARDED_ALLOW_IPS: "192.168.1.1,10.0.0.1"`
