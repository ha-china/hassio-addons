...
我在业余时间维护这个以及其他 Home Assistant 插件：跟踪上游更改、HA 更改以及在真实硬件上进行测试需要花费大量时间（以及一些金钱）。我经常使用大约 5-10 个我 >110 个插件，所以我安装了测试机器（并购买了一些我不使用的测试服务，如 vpn），以便进行故障排除和改进插件。
...
警告：armv7 只支持到版本 0.4.3！它不会随着后续版本进行更新
...
Mealie 是一个自托管的食谱管理和餐食计划器，具有 RestAPI 后端和基于 Vue 的响应式前端应用程序，为全家提供愉悦的用户体验。
此 Mealie 1.0 插件基于 hendrix04 的组合 [docker 镜像](https://hub.docker.com/r/hendrix04/mealie-combined)。
此插件基于 [docker 镜像](https://hub.docker.com/r/hkotel/mealie) 来自 hay-kot。
...
要配置 Gunicorn 的 `--forwarded-allow-ips` 设置以信任反向代理，请手动将 `FORWARDED_ALLOW_IPS` 条目添加到您的插件配置中（以逗号分隔的 IP）。它是可选的，并且从默认选项选项卡中隐藏。例如：`FORWARDED_ALLOW_IPS: "192.168.1.1,10.0.0.1"`
