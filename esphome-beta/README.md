...
该应用允许您编写配置文件，无需编程经验即可将微控制器直接转换为通过 Home Assistant 的智能家居设备。您只需编写 YAML 配置文件，其余工作（空中推送更新、编译）均由 ESPHome 完成。
...
使用 ESPHome，您可以直接从几行 YAML 变成定制固件。例如，要包含 [DHT22][DHT22] 温度湿度传感器，您只需在配置文件中包含 8 行 YAML：
...
然后点击 UPLOAD，传感器便会神奇地出现在 Home Assistant 中：
