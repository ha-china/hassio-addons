...
我在闲暇时间维护此插件及其他 Home Assistant 插件：跟进上游更改、Home Assistant 更改更改以及在真实硬件上进行测试花费了大量时间（以及一些金钱）。我使用了我 110 多个插件中的 5-10 个，因此我经常安装测试机器（并购买一些测试服务，如 vpn），以便我自己不使用这些资源来诊断和改进插件。

如果这个插件为您节省了时间或简化了设置，您的支持将使我很感激！
...
_感谢所有人给我的仓库点了星！请点击下方的图片来给其点星，它将显示在右上角。谢谢！_
...
[emby](https://emby.media/) 将个人媒体库中的视频、音乐、直播电视和图片进行组织，并将它们流媒体传输到智能电视、流媒体盒子和移动设备。此容器打包为独立的 emby 媒体服务器。
...
使用插件的 `env_vars` 选项传递额外的环境变量（大小写均可的名称）。详情请见 https://github.com/alexbelgium/hassio-addons/wiki/Add-Environment-variables-to-your-Addon-2。

Webui 可在 `<your-ip>:8096` 或通过 ingress 在 Home Assistant 中找到。
...
| `cifsdomain` | str | | 网络共享的 SMB  domains |
...
1. 将我的插件存储库添加到您的 Home Assistant 实例中（在 supervisor 插件存储中点击右上角，或如果您已配置我的 HA，则点击下方的按钮）
   [![Open your Home Assistant instance and show the add add-on repository dialog with a specific repository URL pre-filled.](https://my.home-assistant.io/badges/supervisor_add_addon_repository.svg)](https://my.home-assistant.io/redirect/supervisor_add_addon_repository/?repository_url=https%3A%2F%2Fgithub.com%2Falexbelgium%2Fhassio-addons)
1. 安装此插件。
1. 点击 `Save` 按钮以保存您的配置。
1. 启动插件。
1. 检查插件日志以查看一切是否顺利进行。
1. 仔细根据您的需求配置插件，具体官方文档请见。
