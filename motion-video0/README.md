...
这个 _插件_ 针对以下架构构建，并在 Docker Hub 上可用，例如 [`amd64`](https://hub.docker.com/repository/docker/dcmartin/amd64-addon-motion-video0) 版本。
...
+ `client` - 用于监听的 `device` 标识符；可以是 `+` 表示所有设备；_默认_：`+`
...
+ `name` - 根据 _插件_ 定义的摄像头标识符（注意：MQTT 主题 **保留** 字符）
...
+ `w3w` - 根据 [What3Words](http://what3words.com) 识别的摄像头位置
...
所有发送的 `MQTT` 消息都以指定的 `group` 开头的主题开头，例如：
...
# 其他信息
Motion 包有关于可用参数的详细 [文档][motiondoc]。几乎所有参数都可用。
