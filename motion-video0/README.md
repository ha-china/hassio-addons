...
此 [Home Assistant](http://home-assistant.io) 插件利用 [motion 项目](https://motion-project.github.io/)、[YOLO](https://pjreddie.com/darknet/yolo/) 以及其他 AI 技术，用于检测和分类图像中的实体。_motion 项目_ 软件提供了广泛的功能，可以从各种来源捕获视频流，包括 `RSTP`、`HTTP` 和 `MJPEG` 网络摄像头。本地连接的视频源也受支持（例如 `/dev/video0`）。
...
+ `device` - 组中每个设备的唯一标识符；**不应**使用保留的 MQTT 字符（例如 `-,+,#,/`）
...
+ `w3w` - 通过 [What3Words](http://what3words.com) 识别的摄像头位置
...
Motion 包含广泛的 [文档](https://motion-project.github.io/motion_config.html) 关于可用参数。几乎所有参数都是可用的。
