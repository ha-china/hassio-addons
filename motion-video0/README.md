...
这个 [Home Assistant](http://home-assistant.io) 插件利用了 [motion 项目](https://motion-project.github.io/)、[YOLO](https://pjreddie.com/darknet/yolo/) 以及其他 AI 技术，用于检测和分类图像中的实体。_motion 项目_ 软件提供了一套广泛的功能，用于从各种来源捕获视频流，包括 `RSTP`、`HTTP` 和 `MJPEG` 网络摄像头。本地连接的视频源也受支持（例如 `/dev/video0`）。
...
这个 _插件_ 适用于以下架构，并在 Docker Hub 中可用，例如 [`amd64`](https://hub.docker.com/repository/docker/dcmartin/amd64-addon-motion-video0) 版本。
...
所有发送的 `MQTT` 消息都以指定的 `group` 开头的主题为前缀，例如：
...
# 其他信息
Motion 包提供了广泛的 [文档](https://motion-project.github.io/motion_config.html) 关于可用参数。几乎所有参数都是可用的。
JSON 配置选项使用与 Motion 文档中相同的名称提供。

## 更新日志 & 发布
发布基于语义版本控制，并使用 `MAJOR.MINOR.PATCH` 格式。简而言之，版本将根据以下内容进行增量：
