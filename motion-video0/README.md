...
此 [Home Assistant](http://home-assistant.io) 插件利用 [motion 项目](https://motion-project.github.io/)、[YOLO](https://pjreddie.com/darknet/yolo/) 以及其他 AI 技术，用于检测和分类图像中的实体。_motion 项目_ 软件提供了一套广泛的功能，可以从各种来源捕获视频流，包括 `RSTP`、`HTTP` 和 `MJPEG` 网络摄像头。本地连接的视频源也得到支持（例如 `/dev/video0`）。

## 相关

此插件与以下组件和服务交互：

1. [`motion-ai`](http://github.com/dcmartin/motion-ai/tree/master/README.md) - 用于与 `motion` _插件_一起使用的自动 Home Assistant 配置器（见下文）。
...
+ `mqtt` - `MQTT` 中继器的 IP 地址（FQDN）、用户名、密码和端口
+ `group` - 摄像头子集的标识符；_默认_：`motion`
+ `device` - 组中每个设备的唯一标识符；**不得**使用保留的 MQTT 字符（例如 `-,+,#,/`）
+ `client` - 用于侦听 `device` 的标识符；可以是 `+` 以指示所有设备；_默认_：`+`
...
所有发送的 `MQTT` 消息都以指定的 `group` 开头的主题，例如：
...
# `MQTT`
需要 `MQTT` 中继器；默认的 Mosquitto _插件_ 足够，但必须配置适当的认证，例如：
...
# 其他信息
Motion 软件包具有关于可用参数的详细 [文档](https://motion-project.github.io/motion_config.html)。几乎所有参数都是可用的。
