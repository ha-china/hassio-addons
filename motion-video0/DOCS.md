...
此插件的配置关键选项；_组_、_设备_和_摄像头_选项用于标识计算机集合及其相关视频来源；`MQTT` 代理是接收此插件消息以及启用 [`motion-ai`](http://github.com/dcmartin/motion-ai) 解决方案所必需的。
...
## `mqtt`
+ `mqtt.host` - 如果在局域网中使用单个 MQTT 代理，请使用 TCP/IPv4 地址，例如 `192.168.1.40`。
