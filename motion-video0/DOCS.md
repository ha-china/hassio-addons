# ✨ `motion` 经典配置
`motion` 插件处理视频信息，生成运动检测 JSON 事件、多帧 GIF 动画，以及检测、分类和标注的代表性帧（注意：需要 Open Horizon `yolo4motion` 服务）。此插件设计用于与各种来源配合使用，包括：
...
此插件的配置关键选项；`_group_`、`_device_` 和 `_camera_` 选项标识计算机集合及其相关的视频来源；`MQTT` 布局器是接收此插件消息的**必需**条件，同时也能启用 [`motion-ai`](http://github.com/dcmartin/motion-ai) 解决方案。
...
## `mqtt`
+ `mqtt.host` - 如果在局域网中使用单个 MQTT 布局器，请使用 TCP/IPv4 地址，例如 `192.168.1.40`。
...
### 摄像头类型：`network`
网络摄像头直接通过 `MJPEG`、`RTSP` 或 `HTTP` 传输被插件访问；不同摄像头需要特定的传输和相关属性，例如凭证。示例列表如下：
