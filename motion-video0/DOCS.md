# 🕹️ `motion` 经典配置
`motion` 插件处理视频信息，生成运动检测 JSON 事件、多帧 GIF 动画，以及检测、分类和注释的代表性帧（注：需要 Open Horizon `yolo4motion` 服务）。此插件设计用于与多种来源配合使用，包括：

+ `3GP` - 运动检测 Web 摄像头（例如 Linksys WCV80n）；通过 `FTP` 社区插件接收
+ `MJPEG` - 提供运动 JPEG 实时流的网络摄像头
+ `V4L2` - 用于直接连接摄像头的视频 Linux（v2），例如索尼 PlayStation3 Eye 摄像头或树莓派 v2

配置此插件的键选项；_组_、_设备_和_摄像头_选项标识计算机集合及其相关的视频来源；`MQTT` 布局器是**必需**的，用于接收来自此插件的消息，并启用 [`motion-ai`](http://github.com/dcmartin/motion-ai) 解决方案。
...
✅ **标识符**只能**仅**包含小写字母（`a-z`）、数字（`0-9`）和下划线（`_`）。

## `mqtt`
+ `mqtt.host` - 如果在局域网中使用单个 MQTT 布局器，使用 TCP/IPv4 地址，例如 `192.168.1.40`。
+ `mqtt.port` - TCP/IP 端口号；默认：`1883`
+ `mqtt.username` - 访问布局器的凭证；默认：无
+ `mqtt.password` - 访问布局器的凭证；默认：无

## `cameras` **必需**
+ `cameras[].name` - 摄像头的标识符；在设备组和组内唯一；默认：无
