# ✨ `motion` 经典配置 
`motion` 插件处理视频信息，生成运动检测的 JSON 事件、多帧 GIF 动画，以及包含检测、分类和注释的代表性帧（注：需要 Open Horizon `yolo4motion` 服务）。此插件设计用于与多种来源一起工作，包括：

+ `3GP` - 运动检测的 WebCams（例如 Linksys WCV80n）；通过 `FTP` 社区插件接收
+ `MJPEG` - 提供运动 JPEG 实时流的网络可访问相机
+ `V4L2` - 用于直接连接的相机（例如 Sony PlayStation3 Eye 相机或 RaspberryPi v2）的 Linux 视频接口（v2）

此插件的配置关键选项；`_group_`、`_device_` 和 `_camera_` 选项标识计算机集合和相关联的视频来源；`MQTT` 中继器是**必需**的，用于接收来自此插件的消息，以及启用 [`motion-ai`](http://github.com/dcmartin/motion-ai) 解决方案。

## `group`、`device` 和 `client`
标识符区分集合以及个体；相机标识符必须在组内唯一。
...
## `cameras` **必需**
+ `cameras[].name` - 相机的标识符；在 `_device_` 和 `_group_` 中唯一；默认：无
+ `cameras[].type` - `netcam`、`ftp`、`mqtt`；`local` 仅适用于 [motion-video0](http://github.com/dcmartin/addon-motion-video) 版本
+ `cameras[].w3w` - 相机的位置，用三个字符串表示；来自 what3words.com；无使用 `[]`

## `cameras` _可选_
+ `cameras[].top` - 图标顶部位置的百分比（0-95）
+ `cameras[].left` - 图标左侧位置的百分比（0-95）
+ `cameras[].icon` - 来自 materialdesignicons.com 的相机图标（注：不包括 `mdi:` 组件）
...
# 详细说明
下一部分详细说明了可用于配置插件的所有属性。

### 相机类型：`network`
网络相机通过 `MJPEG`、`RTSP` 或 `HTTP` 三种传输方式直接被插件访问；不同的相机需要特定的传输方式和相关属性，例如凭证。以下列出了一些示例：
