## ⚠️ 开启请求 : [✨ [请求] [Joplin] 添加网关 (已开启 2025-06-15)](https://github.com/alexbelgium/hassio-addons/issues/1913) 由 [@aluavin](https://github.com/aluavin)
# Home assistant 插件：Joplin

我利用业余时间维护这个和其他 Home Assistant 插件：跟上上游的变更、HA 的变更，并在真实硬件上测试都需要大量时间（和一些金钱）。我大约使用我 110 多个插件中的 5-10 个，因此我安装了一些用于测试的机器（和一些我自己不使用的测试服务，如 VPN），以便于调试和改进插件。
...
[![加星标仓库列表 for @alexbelgium/hassio-addons](https://raw.githubusercontent.com/alexbelgium/hassio-addons/master/.github/stars2.svg)](https://github.com/alexbelgium/hassio-addons/stargazers)

![下载量趋势](https://raw.githubusercontent.com/alexbelgium/hassio-addons/master/joplin/stats.png)

## 关于

[Joplin Server](https://github.com/laurent22/joplin) 是一个免费、开源的笔记和待办事项同步应用程序，它可以处理大量按笔记本组织的笔记。使用这个服务器，你可以在所有设备上同步所有你的笔记。Joplin 支持端到端加密、Markdown 编辑、网页剪辑扩展，以及与各种云服务的同步。

这个插件基于 [docker 镜像](https://hub.docker.com/r/etechonomy/joplin-server) 的 etechonomy。

感谢 @poudenes 在开发过程中提供的帮助！

## 配置

Web UI 位于 `<你的 IP>:22300`。
...
| `MAILER_ENABLED` | 整数 | | 启用邮件服务（1=true, 0=false） |
...
1. 安装和配置一个 PostgreSQL 插件（例如，MariaDB 插件）
2. 为 Joplin 创建数据库和用户
...
这个插件支持通过 `addon_config` 映射来使用自定义脚本和环境变量：

- **自定义脚本**：请参阅 [在插件中运行自定义脚本](https://github.com/alexbelgium/hassio-addons/wiki/Running-custom-scripts-in-Addons)
- **env_vars 选项**：使用插件的 `env_vars` 选项来传递额外的环境变量（名称可以是大小写混合）。请参阅 https://github.com/alexbelgium/hassio-addons/wiki/Add-Environment-variables-to-your-Addon-2 获取详细信息。
...
3. **配置同步**：设置你的 Joplin 客户端以与服务器同步
4. **可选数据库**：考虑切换到 PostgreSQL 以获得更好的性能
5. **邮件服务**：配置邮件服务以用于用户管理功能
