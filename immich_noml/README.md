...
![Version](https://img.shields.io/badge/dynamic/yaml?label=版本&query=%24.version&url=https%3A%2F%2Fraw.githubusercontent.com%2Falexbelgium%2Fhassio-addons%2Fmaster%2Fimmich_noml%2Fconfig.yaml)
...
直接从您的手机备份照片和视频的解决方案。这是 Immich 的 NoML（无机器学习）版本，专为没有机器学习功能或用户因性能或资源管理原因希望禁用 ML 功能的系统而设计。

此插件基于 imagegenius 的 [docker 镜像](https://github.com/imagegenius/docker-immich)，排除了机器学习组件以减少资源消耗并提高与资源受限系统的兼容性。

## 使用场景

NoML 版本非常适合：

- **资源受限的系统**：在不使用 ML 开销的情况下降低 CPU 和内存使用
- **注重隐私的部署**：不进行人脸识别或物体检测处理
- **简单的照片存储**：基本的照片和视频备份，不使用高级 AI 功能
- **旧硬件**：难以处理机器学习工作负载的系统
- **极简设置**：喜欢基本照片管理而不使用 AI 增强的用户
