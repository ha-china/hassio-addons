...
⚠️ 该项目处于非常活跃的开发中。预期会有错误和变更。请勿将其作为存储照片和视频的唯一方式！（开发者提醒）

我在业余时间维护这个和其他 Home Assistant 插件：跟踪上游变更、Home Assistant 变更以及在真实硬件上进行测试需要花费大量时间（以及一些金钱）。我经常使用我超过 110 个插件中的 5-10 个，所以我安装了测试机器（并购买了某些我本人不使用的测试服务，如 vpn），以便进行故障排除和改进插件。
...
- **资源受限的系统**：降低 CPU 和内存使用，无需 ML 过载
- **注重隐私的部署**：无面部识别或对象检测处理
- **简单的照片存储**：基本的照片和视频备份，无高级 AI 功能
- **旧硬件**：难以处理机器学习工作负载的系统
- **极简设置**：更喜欢基本照片管理而不需要 AI 增强的用户
