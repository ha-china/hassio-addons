...
⚠️ 该项目处于非常活跃的开发中。请预期可能出现错误和更改。请不要将其作为存储照片和视频的唯一方式！ (来自开发者)


我在业余时间维护这个和其他 Home Assistant 插件：跟进上游更改、HA 更改以及在真实硬件上进行测试需要花费大量时间（以及一些金钱）。我经常使用我超过 110 个插件中的 5-10 个，所以我安装了测试机器（并购买了一些我本人不使用的测试服务，如 vpn），以便进行故障排除和改进插件
...
- **注重隐私的部署**：没有面部识别或对象检测处理
- **简单的照片存储**：基本的照片和视频备份，没有高级 AI 功能
- **旧硬件**：难以处理机器学习工作负载的系统
- **简约设置**：更喜欢基本照片管理而不希望使用 AI 优化的用户
...
| `DISABLE_MACHINE_LEARNING` | bool | `false` | 禁用 ML 功能（建议用于 NoML 版本） |
