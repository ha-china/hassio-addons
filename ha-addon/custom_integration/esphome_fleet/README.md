...
这个子树是Home Assistant的**自定义集成**，与ESPHome附加组件（`ha-addon/server/`中的aiohttp服务器）配对。它将附加组件转变为高级HA公民：

- 通过Supervisor或`_esphome-fleet._tcp` mDNS（`config_flow.py`）发现附加组件。
- 每30秒通过`DataUpdateCoordinator`（`coordinator.py`）轮询`/ui/api/*`，并通过`ws_client.py`的实时WebSocket事件流进行即时更新。
...
仓库、Docker镜像名称、附加组件slug和内部Python模块保持其原始的`distributed_esphome` / `esphome-dist-*`形式——更改这些将迫使每个现有安装进行迁移。**面向用户的**品牌是“Fleet for ESPHome”（见`CLAUDE.md`中的命名约定部分）。集成`domain: esphome_fleet`选择面向用户的名字，因此HA用户在他们的UI中看到一个干净的`esphome_fleet.compile`服务和`esphome_fleet`集成，而没有`distributed_`前缀泄露出来。
...
此目录中的`icon.png`（64×64）和`logo.png`（192×192）是附加组件顶级`../../icon.png` / `../../logo.png`的副本，与集成一起保留，以便任何检查它的人都知道哪个艺术品代表集成。

**注意：**Home Assistant的集成UI**不会渲染自定义集成的本地品牌**。标志来自[`home-assistant/brands`](https://github.com/home-assistant/brands)仓库——HA的前端从`https://brands.home-assistant.io/<domain>/icon.png`获取它们。要使标志在集成页面上真正可见，需要向该仓库提交一个PR，该PR需要包含以下内容：
...
我们当前在磁盘上的资产是64×64 / 192×192，因为这是Supervisor附加组件目录所需的大小。brands仓库要求的较大尺寸需要在打开PR之前从源艺术作品重新生成——在`WORKITEMS-future.md`中跟踪作为后续任务。
---
