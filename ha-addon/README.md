...
桥接音乐助手通过 Sendspin 协议连接蓝牙扬声器。
从音乐助手流式传输音频到连接到您的 Home Assistant 主机的任何蓝牙 A2DP 扬声器。

## 关于

此附加组件允许您在音乐助手中使用蓝牙扬声器作为音频输出播放器。它通过 Sendspin 协议连接到音乐助手，并通过 PulseAudio/PipeWire 路由音频流到配对的蓝牙设备。
...
- HA Ingress 总是使用固定的轨道特定端口（`8080` 稳定，`8081` RC，`8082` beta）。自定义 `WEB_PORT` 仅添加额外的直接监听器；它不会替换 Ingress。
- 在附加组件模式下，身份验证始终由 Home Assistant / Ingress 强制执行；独立密码切换在此处不适用。
- 仅通过附加组件/Ingress 流才能通过静默 Home Assistant 令牌引导音乐助手，因此相关的 UI 辅助程序故意设置为附加组件范围。
- **不要**同时在多个附加组件版本中配置相同的蓝牙扬声器。
