...
- 本存储库中检查的附加组件清单是 **稳定** 的 Home Assistant 附加组件版本。
...
- HA Ingress 总是使用固定的轨迹特定端口（`8080` 稳定，`8081` RC，`8082` beta）。自定义 `WEB_PORT` 仅添加额外的直接监听器；它不会替换 Ingress。
- 在附加组件模式下，身份验证始终由 Home Assistant / Ingress 强制执行；独立的密码切换在此处不适用。
- 仅通过附加组件/Ingress 流才能使用静默 Home Assistant 令牌引导 Music Assistant，因此相关的 UI 辅助程序有意限制在附加组件范围内。
