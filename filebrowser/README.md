...
我在业余时间维护这个和其他 Home Assistant 扩展：跟进上游更改、HA 更改以及在真实硬件上进行测试需要花费大量时间（以及一些金钱）。我经常使用大约 5-10 个我 >110 个扩展，所以我安装了测试机器（并购买了某些测试服务，如 vpn），这些服务我自己并不使用，以便进行故障排除和改进扩展。
...
基于 Web 的文件管理界面，提供了一种安全的方式来浏览、上传、下载、编辑和管理 Home Assistant 系统上的文件。Filebrowser 提供了一个干净、现代的界面，通过 Web 浏览器处理文件，支持多种文件格式、预览功能以及全面的文件操作。
...
1. 将我的扩展仓库添加到您的 home assistant 实例中（在监督器扩展存储的右上角，或点击下面的按钮如果您已经配置了我的 HA）
