...
我在业余时间维护这个和其他 Home Assistant 插件：跟进上游更改、HA 更改以及在实际硬件上的测试都需要花费大量的时间（和一些金钱）。我经常使用大约 5-10 个我的 >110 个插件，所以我安装了测试机器（并购买了某些我自身不使用的测试服务，如 VPN）来调试和改进插件。
...
_感谢每一位为我仓库加星的人！要加星，请点击下面的图片，然后它就会显示在右上角。谢谢！_
...
基于 Web 的文件管理界面，提供了一种安全的方式在您的 Home Assistant 系统上浏览、上传、下载、编辑和管理文件。Filebrowser 通过网络浏览器提供了一个干净、现代的界面来处理您的文件，支持多种文件格式、预览功能以及全面的文件操作。
...
Web 界面可以在 `<your-ip>:8071` 或通过使用入口的 Home Assistant 侧边栏中找到。
