...
我在业余时间维护这个以及其他 Home Assistant 插件：跟进上游更改、Home Assistant 更改以及在实际硬件上进行测试需要花费很多时间（以及一些金钱）。我经常使用我超过 110 个插件中的 5-10 个，所以我安装了测试机器（并购买了一些我自己不使用的测试服务，如 VPN）来调试和改进插件。
...
基于 Web 的文件管理界面，提供了一种安全的方式来浏览、上传、下载、编辑和管理 Home Assistant 系统上的文件。Filebrowser 通过 Web 浏览器提供了一种干净、现代的界面来处理您的文件，支持多种文件格式、预览功能以及全面的文件操作。
...
Web UI 可在 `<your-ip>:8071` 或使用入口时通过 Home Assistant 侧边栏访问。
