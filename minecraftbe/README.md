...
你可能想在深夜创建一个服务来重启扩展，以便更新 Minecraft 版本（见下文）
...
5. 启动扩展。
6. 检查扩展的日志，以查看是否一切顺利。
7. 编辑你想要在 /share/minecraftbe/ 中的 server/permissions/whitelist 属性，然后重启扩展。注意，你无法在 server.properties 中更改端口，因为它会被某些原因覆盖。然而，你可以在 homeassistant 的扩展配置标签中更改端口。我只暴露了 IP4 端口。如果需要 IP6，请告诉我。
8. 如果你想要外部访问，请确保将你的外部端口转发到 homeassistant IP。
