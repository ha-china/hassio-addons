...
你可能想创建一个服务，在深夜重启插件以更新 Minecraft 版本（见下文）
...
5. 启动插件。
6. 检查插件的日志，以查看一切是否顺利。
7. 编辑你想要在 /share/minecraftbe/ 中的服务器/权限/白名单属性，然后重启插件。注意，你无法在 server.properties 中更改端口，因为它会被某种原因覆盖。然而，你可以在 Home Assistant 的插件配置标签中更改端口。我只暴露了 IP4 端口。如果需要 IP6，请告诉我。
