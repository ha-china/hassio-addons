...
我在业余时间维护这个以及其他 Home Assistant 插件：跟踪上游更改、HA 更改以及在真实硬件上进行测试需要花费很多时间（以及一些金钱）。我经常使用大约 5-10 个我 >110 个插件，所以我安装了测试机器（并购买了一些我自身不使用的测试服务，如 vpn），以便进行故障排除和改进插件。
...
_感谢每一位给我的仓库点星的人！要星标它，请点击下面的图片，然后它就会出现在右上角。谢谢！_
...
[Tandoor 烹饪食谱](https://github.com/TandoorRecipes/recipes)，由 [vabene1111](https://github.com/vabene1111) 制作，旨在为那些想要与家人和朋友分享食谱或以整洁的方式存储食谱的人。存在基本的权限系统，但这个应用程序并不打算作为一个公开页面运行。
...
除了以下选项外，所有配置都可以通过应用程序 WebUI 完成。
...
| 选项 | 类型 | 默认值 | 描述 |
|------|------|---------|-------------|
| `SECRET_KEY` | str | `YOUR_SECRET_KEY` | **必需的**：Django 安全密钥 |
| `ALLOWED_HOSTS` | str | | **必需的**：逗号分隔的 Home Assistant 网址，用于入口 |
| `DB_TYPE` | list | `sqlite` | 数据库类型（sqlite 或 postgresql_external） |
| `DEBUG` | list | `0` | 调试模式（0=正常，1=调试） |
| `externalfiles_folder` | str | | 外部食谱文件导入的文件夹 |
| `POSTGRES_HOST` | str | | PostgreSQL 主机（必需的，用于 postgresql_external） |
| `POSTGRES_PORT` | str | | PostgreSQL 端口（必需的，用于 postgresql_external） |
| `POSTGRES_USER` | str | | PostgreSQL 用户名（必需的，用于 postgresql_external） |
| `POSTGRES_PASSWORD` | str | | PostgreSQL 密码（必需的，用于 postgresql_external） |
| `POSTGRES_DB` | str | | PostgreSQL 数据库名称（必需的，用于 postgresql_external） |
| `AI_MODEL_NAME` | str | | 用于配置 LLM，支持的提供商可以在 [这里](https://docs.litellm.ai/docs/providers/) 找到 |
| `AI_API_KEY` | str | | 访问 LLM 的 API 密钥 |
| `AI_RATELIMIT` | str | | LLM 访问速率限制，指定为 [DRF 语法](https://www.django-rest-framework.org/api-guide/throttling/) |
...
![截图](https://github.com/TandoorRecipes/recipes/raw/develop/docs/preview.png)

[repository]: https://github.com/alexbelgium/hassio-addons

## 外部食谱文件
目录 /config/addons_config/tandoor_recipes/externalfiles 可用于将外部文件导入 Tandoor。您可以将此与 Docker 中的 /opt/recipes/externalfiles 映射。
请参阅以下说明：https://docs.tandoor.dev/features/external_recipes/
---
