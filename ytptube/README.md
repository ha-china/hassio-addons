...
yt-dlp 的网页界面，支持播放列表和频道功能（https://github.com/arabcoders/ytptube）。

_感谢所有为我点赞的仓库！要点赞，请点击下面的图片，然后它就会显示在右上角。谢谢！_
...
* 用逗号分隔的多个 URL 排队。
* 预设系统可重复使用常用的 yt-dlp 选项。
* 简单的文件浏览器。`默认禁用`。
* 内置视频播放器 **支持侧载外部字幕**。
* 新增 `POST /api/history` 端点，允许一次发送一个或多个链接。
...
* 支持基本模式 WebUI，方便非技术用户使用，隐藏了大多数正常功能。
