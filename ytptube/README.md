...
_感谢所有star我的仓库的人！要star它，请点击下面的图片，然后它就会显示在右上角。谢谢！_
...
* SQLite作为数据库后端。
* 基本认证支持。
* 支持curl_cffi，请参阅[yt-dlp文档](https://github.com/yt-dlp/yt-dlp?tab=readme-ov-file#impersonation)。
* 支持WebUI的基本模式，适用于非技术用户，隐藏了大多数正常功能。
* 容器中捆绑的工具：curl-cffi、ffmpeg、ffprobe、aria2、rtmpdump、mkvtoolsnix、mp4box。
* 自动重新排队即将到来的直播流。
* 根据自定义定义的条件应用`yt-dlp`选项。
* 自定义浏览器扩展、书签和iOS快捷方式，可以将链接发送到YTPTube实例。

## 安装

此扩展的安装有几个额外步骤。

1. [将我的Hass.io扩展仓库添加到您的Hass.io实例中][repository]。
1. 安装此扩展。
1. 点击`保存`按钮以存储您的配置。
