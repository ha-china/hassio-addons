...
我在业余时间维护这个以及其他 Home Assistant 插件：跟上上游更改、HA 更改以及在真实硬件上进行测试都需要花费大量时间（以及一些金钱）。我经常使用我超过 110 个插件中的 5-10 个，所以我安装了测试机器（并购买了一些我自己不使用的测试服务，如 VPN），以便进行故障排除和改进插件。
...
| 选项 | 类型 | 默认值 | 描述 |
|------|------|---------|-------|
| `CUSTOM_OPTIONS` | 字符串 | | 自定义 CLI 选项（例如，`--name Homeassistant`） |
| `DROPBOX_TOKEN` | 字符串 | | Dropbox 集成令牌（见 https://cloudcmd.io/） |
...
- **env_vars 选项**：使用插件的 `env_vars` 选项来传递额外的环境变量（使用大写或小写名称）。见 https://github.com/alexbelgium/hassio-addons/wiki/Add-Environment-variables-to-your-Addon-2 获取详细信息。
