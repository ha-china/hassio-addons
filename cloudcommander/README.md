...
我在业余时间维护这个和其他 Home Assistant 插件：跟踪上游变更、HA 变更以及在实际硬件上进行测试需要花费很多时间（还有一些钱）。我经常使用大约 5-10 个我 >110 个插件，所以我安装了测试机器（并购买了些我自己不用的测试服务，例如 vpn）来调试和改进插件。
...
[Cloud Commander](https://github.com/coderaiser/cloudcmd) 是一个带有控制台和编辑器的网络文件管理器。
...
- **env_vars 选项**：使用插件的 `env_vars` 选项来传递额外的环境变量（大写或小写名称）。见 https://github.com/alexbelgium/hassio-addons/wiki/Add-Environment-variables-to-your-Addon-2 获取详细信息。

## 安装

此插件的安装非常简单，与安装任何其他 Hass.io 插件没有区别。

1. 将我的插件仓库添加到您的 Home Assistant 实例中（在右上角的 supervisor 插件存储中，或点击下面的按钮如果您已经配置了我的 HA）
