...
这是一个音乐助手的特殊开发插件，允许开发人员快速测试特定的分支、拉取请求，甚至直接在 Home Assistant 中测试音乐助手的分叉。
...
> **注意**：当 `server_repo` 留空或为空时，插件将安装 GitHub 发布中的最新夜间发布轮。这是最快的选项，因为无需服务器构建。
...
> **注意**：当 `frontend_repo` 留空或为空时，前端构建将**完全跳过**。这显著减少了启动时间，并且当您只需要测试后端功能时是理想的。将使用与服务器安装捆绑的前端。
...
frontend_repo: pr-789
```
从源代码构建服务器（PR #456）和前端（PR #789）。完全控制，用于综合测试。
...
构建时间取决于您的配置：
- **都为空**（`server_repo: ""` 和 `frontend_repo: ""`）：最快 - 无需构建，使用最新的夜间发布
- **仅指定 `server_repo`**：中等 - 仅构建服务器，跳过前端（适用于后端测试）
- **都指定**：最慢 - 从源代码构建服务器和前端（完全开发模式）
...
| 前端     | 捆绑        | 捆绑                  | 从源代码构建          |
| 更新     | 自动        | 手动（重启）          | 手动（更改配置）      |
| 用途     | 生产        | 快速后端测试          | 完全开发/测试        |

**配置模式：**
- **快速模式**：两个仓库都为空 - 使用最新的夜间发布，无需构建
