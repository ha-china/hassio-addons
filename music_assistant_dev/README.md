# Music Assistant 开发者插件

这是一个专门为 Music Assistant 开发的设计插件，允许开发者快速测试特定的分支、拉取请求，甚至直接在 Home Assistant 中测试 Music Assistant 的分支。
...
1. 从指定的 Git 源（分支、PR 或分支）构建并安装服务器
2. 从指定的 Git 源（分支、PR 或分支）构建并安装前端
3. 使用您的自定义代码启动 Music Assistant

构建过程：

1. 从指定的 Git 引用安装服务器软件包
2. 根据其构建过程构建前端（npm build）
...
> **注意**： 当 `server_repo` 为空或空白时，插件将从 GitHub 发布中安装最新的夜间发布轮。这是最快的选项，因为不需要构建服务器。

### 前端仓库配置

使用 `frontend_repo` 选项指定要安装的 Music Assistant 前端的版本：

**格式**： 与服务器_repo 相同 - `owner/repo@reference` 或只是 `reference`
...
> **注意**： 当 `frontend_repo` 为空或空白时，前端构建将**完全跳过**。这会显著减少启动时间，并且当您只需要测试后端功能时非常适合。将使用与服务器安装捆绑的前端。
...
frontend_repo: pr-789
```
从源代码构建服务器（PR #456）和前端（PR #789）。全面的控制，用于综合测试。
...
构建时间取决于您的配置：
- **两者都为空**（`server_repo: ""` 和 `frontend_repo: ""`）：最快 - 无需构建，使用最新的夜间发布
...
- 如果您需要在加载提供程序之前启动 Music Assistant，请设置 `safe_mode: true`
- 用于调试任何启动问题
...
- 确保指定的 Git 引用存在
- 检查分支中是否存在依赖冲突
- 前端构建需要 Node.js - 构建失败可能指示前端代码不兼容
...
这是一个开发者工具，不适合普通用户使用。如果您遇到问题：

- 检查插件日志
- 验证您的 Git 引用是否正确
- 首先使用默认分支进行测试
- 在 Music Assistant 开发者 Discord 通道中提问

## 与常规插件的差异

| 功能      | 常规插件    | 开发者插件（夜间模式） | 开发者插件（源代码模式） |
| ------------ | ----------------- | ------------------------- | ------------------------ |
...
**配置模式**：
- **快速模式**： 两者都为空 - 使用最新夜间发布，无需构建
- **后端开发模式**： 仅指定 `server_repo` - 构建服务器，使用捆绑前端
- **完全开发模式**： 两者都指定 - 从源代码构建所有内容
