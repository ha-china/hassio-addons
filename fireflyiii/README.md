...
我利用业余时间维护此及其他 Home Assistant 插件：跟踪上游更改、Home Assistant 更改更新以及在真实硬件上测试耗费了大量时间（和一些金钱）。我使用了大约 5-10 个我拥有的 >110 个插件中的一部分，因此我会定期安装测试机器（并购买一些我不自己使用的测试服务，如 vpn）来调试和改进插件。

如果这个插件为您节省了时间或让您的设置更容易，我将非常感激您的支持！
...
_感谢大家为我仓库星标！要星标它，点击下方图片，它将出现在右上角。谢谢！_
...
["Firefly III"](https://www.firefly-iii.org) 是一个针对个人财务的（自托管）管理器。它可以帮助您追踪支出和收入，从而让您花得更少，存得多。

该插件基于 docker 镜像 https://hub.docker.com/r/fireflyiii/core。

## 配置

使用插件的 `env_vars` 选项传入额外的环境变量（大小写均可）。详见 https://github.com/alexbelgium/hassio-addons/wiki/Add-Environment-variables-to-your-Addon-2。

Webui 可以通过 <http://homeassistant:PORT> 访问，或通过侧边栏使用 Ingress 访问。
除了以下选项外，配置可以通过应用 Webui 完成。

**⚠️ 重要**：首次启动前必须更改您的 `APP_KEY`！之后将无法更改，除非重置数据库。

### 选项

| 选项 | 类型 | 默认值 | 描述 |
|------|------|--------|------|
| `APP_KEY` | str | `CHANGEME_32_CHARS_EuC5dfn3LAPzeO` | **关键**：32 字符加密密钥 - 首次运行前必须更改！ |
| `CONFIG_LOCATION` | str | `/config/addons_config/fireflyiii/config.yaml` | 额外配置文件位置 |
...
| `DB_USERNAME` | str | | 数据库用户名 (如果已设置，将覆盖 MariaDB 插件服务发现) |
| `DB_PASSWORD` | str | | 数据库密码 (如果已设置，将覆盖 MariaDB 插件服务发现) |
...
1. 将我的插件存储库添加到您的 Home Assistant 实例（在 supervisor 插件商店顶部右侧，或者如果您已配置了我的 HA，则点击下面的按钮）
