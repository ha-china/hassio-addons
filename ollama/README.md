...
请注意，此插件需要使用 CPU 加速或实验性的 Nvidia GPU 支持（如果对您有效，请报告一下！）。对于 ROCm，支持仍在等待中。

## 模型目录

所有下载的模型默认存储在 `/share/ollama`。由于历史原因，您也可以将其配置为 `/config/ollama`。请确保您有足够的空间。您可以选择 `/data/ollama` 以保持备份小，因为这个路径不包括在插件备份中。
