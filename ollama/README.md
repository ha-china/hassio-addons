...
请注意，此附加组件使用 CPU 加速运行，或支持实验性的 Nvidia GPU（如果对您有效，请报告！）。对于 ROCm，支持仍在待定中。

## 模型目录

所有下载的模型默认存储在 `/share/ollama`。出于历史原因，您也可以将其配置为 `/config/ollama`。请确保您有足够的空间可用。
...
如果您想更改模型，请删除集成（不是附加组件！）并重启配置集成的流程。
