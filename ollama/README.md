...
请注意，此插件使用 CPU 加速或实验性的 Nvidia GPU 支持（如果它对您有效，请报告！）。对于 ROCm 的支持仍在等待中。

## 模型目录

所有下载的模型默认存储在 `/share/ollama`。由于历史原因，您也可以将其配置为 `/config/ollama`。请确保您有足够的空间可用。您可以选择 `/data/ollama` 以保持您的备份小巧，因为此路径已被排除在插件备份之外。
...
如果您想更改模型，请删除集成（而不是插件！），然后重新启动集成的配置过程。
