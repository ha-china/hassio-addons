...
Z-Wave JS UI 应用提供了一个额外的控制面板，允许您对 Z-Wave 网络的各个方面进行配置。它提供了一个解耦的网关，该网关可以使用 Z-Wave JS 套接字（用于 Home Assistant Z-Wave JS 集成）和 MQTT（甚至可以同时使用）进行通信。

一些优势和用例：

- 与 Home Assistant Z-Wave JS 集成兼容。
- 在 Home Assistant 重启期间，您的 Z-Wave 网络将继续运行。
- 您可以直接像使用 Node-RED 一样使用 Z-Wave 网络，同时它也对 Home Assistant 可用。
- 允许基于 [ESPHome.io][esphome] 的 ESP 设备直接与您的 Z-Wave 网络响应或协作。
- 在发现时会自动配置 Mosquitto 应用。
