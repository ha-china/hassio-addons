...
1. HOMEPAGE_ALLOWED_HOSTS
   - 默认情况下，这个插件使用通配符 (*) 作为 HOMEPAGE_ALLOWED_HOSTS。
   - 对于典型的 Home Assistant 设置来说这是安全的，因为插件在隔离的容器中本地运行，并且不会直接暴露给互联网。
   - 安全提示：如果你将来将这个容器暴露给外部网络，使用通配符可能会允许不想要的请求。在这种情况下，建议明确指定允许的主机名。
...
3. 自定义图标和图片
   ...
   - 由于在haos中无法挂载 `/app/public/icons` 以供homepage使用，这里有一个替代方案，更容易使用。
...
4. 版本编号：
   - 使用 **Vx.x.x.x** 格式。
   - 前三个数字遵循官方Homepage版本（例如：`1.5.0`）。
   - 最后一个数字用于此Home Assistant插件的更改（例如：`1.5.0.1`）。
...
## 编辑Homepage配置文件
1. 使用文件编辑插件或通过SFTP连接到你的Home Assistant。
2. 导航到 addon_config 文件夹。
3. 打开Homepage插件的文件夹，例如，xxxxxxx_homepage（xxxxxxx是随机数字）。
4. 文件夹结构应如下：/addon_config/abcd123_homepage/。
...
| 问题 | 可能原因 | 解决方法 |
|---------|----------------|----------|
| **插件启动后只显示“开始”按钮** | Home Assistant Supervisor UI 在首次安装后有时无法正确刷新 | 刷新页面（F5）或再次点击 **开始**。然后，完整的控制按钮（`停止`、`重启`、`卸载`、`重建`、`打开WebUI`）将出现。 |
| **使用 `/local/homepage/icons/...` 时出现404** | 文件不在正确的宿主机目录中，或者 Home Assistant 尚未重新加载静态文件 | 确保文件放置在 `/config/www/homepage/icons/`。重启 Home Assistant（核心）以重新加载 `/local/` 静态文件。 |
| **旧的图标或图片仍然显示** | 浏览器或服务器缓存 | 在浏览器中强制硬刷新（Ctrl+F5）或重命名文件（例如，`favicon_v2.ico`）。 |
| **Homepage上图标/图片未显示** | Homepage容器无法访问 `http://<ha-ip>:8123/local/...` 或URL不正确 | 对于图标/图片，始终在YAML配置中使用带有端口8123的完整Home Assistant URL。 |
| **无法上传文件** | 权限问题或上传位置错误 | 确保你通过SFTP、文件编辑器或其他文件管理器具有对 `/config/www/homepage/...` 的写入权限。 |
| **路径或拼写错误** | 文件夹或文件名拼写错误 | 仔细检查文件夹名称、文件扩展名和大小写敏感性（Linux路径是大小写敏感的）。 |
