...
**最佳且最简单的自托管免费开源网站变更检测、监控和通知服务。Visualping、Watchtower 等的替代品。设计理念简洁——主要目标是免费简单地监控哪些网站有文本变更。免费开源的网页变更检测**
...
- 你有一份非常敏感的 URL 列表需要监控，而且你不想使用付费替代品。（记住，_你_ 是产品）

_需要带有 JavaScript 支持的实际 Chrome 运行器吗？我们支持通过 WebDriver 和 Playwright 获取内容！_

#### 关键特性

- 许多触发过滤器，例如“根据文本触发”、“通过选择器移除文本”、“忽略文本”、“提取文本”，还支持正则表达式！
...
1. 将我的 Hass.io 插件仓库[仓库](https://github.com/jdeath/homeassistant-addons)添加到你的 Hass.io 实例。
1. 安装此插件。
1. 前往 ip:port。入口排序大致可行，但页面无法正确渲染
...
你可以配置 Changedetection.io 使用 Playwright 获取器来获取页面，否则它将使用纯非 JS 内置浏览器获取。使用 Playwright 获取器提供完整的 Changedetection.io 功能，包括 JS 浏览器步骤来获取内容和视觉过滤选择器。
...
两个插件需要在同一台机器上运行。在 Raspberry Pi 4B 上测试了 Home Assistant 2023.5.3/Supervisor 2023.04.1/操作系统 10.1，但应与其他版本兼容，也支持 amd64 设备。

注意：Browserless Chrome 插件在获取网站时资源消耗很大，无论是 RAM 还是 CPU。在 RPi 4B 上运行良好，在较旧的设备上可能较慢。最大同时获取限制为 1。
