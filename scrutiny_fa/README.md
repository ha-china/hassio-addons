# Home Assistant 发行版：Scrutiny

我在业余时间维护此及其他 Home Assistant 发行版插件：跟踪上游更改、Home Assistant 自身的更改以及在实际硬件上进行测试需要大量时间（和一些金钱）。我使用的插件数量在 5-10 个左右（从我拥有的 >110 个插件中），因此我经常安装我自己不使用的测试机器（并购买一些测试服务，如 VPN）来协助调试和改进插件。

如果您使用这个插件节省了时间或使您的设置更容易，您的支持将使我非常感激！
...
_谢谢大家给我仓库点了星！要将其设为星标，请点击下方图片，它将被置于右上角。谢谢！_
...
[Scrutiny](https://github.com/AnalogJ/scrutiny) 是一个硬盘健康仪表板及监控解决方案，它将制造商提供的 S.M.A.R.T 指标与现实世界的故障率相结合。此发行版插件基于 [linuxserver.io](https://www.linuxserver.io/) 的 [docker 镜像](https://hub.docker.com/r/linuxserver/scrutiny)。
...
WebUI 位于 <http://homeassistant:8080>，或通过侧边栏使用入口点访问。
除了以下选项外，还可通过应用程序 WebUI 进行配置。
它会自动挂载所有本地磁盘。

**注意**：仅在使用问题时启用完全访问权限。在所有情况下，SMART 访问都应在无需完全访问权限的情况下正常工作。

### 选项

| 选项 | 类型 | 默认值 | 描述 |
|------|------|--------|------|
| `Updates` | list | `Hourly` | 更新计划 (Quarterly/Hourly/Daily/Weekly/Custom) |
...
- **自定义脚本**：请参阅 [在发行版插件中运行自定义脚本](https://github.com/alexbelgium/hassio-addons/wiki/Running-custom-scripts-in-Addons)
- **env_vars 选项**：使用发行版的 `env_vars` 选项传递额外的环境变量（大写或小写名称均可）。有关详细信息，请访问 https://github.com/alexbelgium/hassio-addons/wiki/Add-Environment-variables-to-your-Addon-2。

## 安装

---

此发行版插件的安装非常简单，与其他任何发行版插件相比没有不同。

1. 将我的发行版插件库添加到 Home Assistant 实例中（在 Supervisor 发行版商店右上角，或者如果您已配置了 Home Assistant，则点击下方的按钮）
...
1. 启动发行版插件。
1. 检查发行版插件的日志，确认一切正常。
1. 打开 WebUI（基于入口点）并调整软件选项。

# 在 Home Assistant 中的集成

---

通过配置.yaml 文件中的 [rest 平台](https://www.home-assistant.io/integrations/rest) 可以与 HA 进行集成。

即使端口未暴露，API 也可在 Home Assistant 的内部网络中访问。使用发行版插件的内部域名（`http://db21ed7f-scrutiny:8080`）从 Home Assistant 或其他发行版插件对其进行查询。如果您需要从本地网络访问 API，请在发行版插件选项中将端口暴露，并将域名替换为 Home Assistant 的 IP 地址。
...
对于详细数据，Wwn 可在 Scrutiny 应用中的每个 HDD 内查找。例如：<http://db21ed7f-scrutiny-fa:8080/api/device/0x50014ee606c14537/details>
