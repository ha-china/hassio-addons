...
我利用业余时间维护此及其他 Home Assistant 附加组件：跟踪上游更改、Home Assistant 更改以及在真实硬件上进行测试耗费了大量时间（以及一些金钱）。我使用超过 110 个附加组件中的 5-10 个，因此我安装了一些我自己不使用的测试机器（并购买了一些测试服务，如 vpn）来诊断和改进这些附加组件。
...
_非常感谢大家星标了我的仓库！要星标它，请点击下方的图片，它将显示在右上角。 Thanks!_
...
[Scrutiny](https://github.com/AnalogJ/scrutiny) 是一个硬盘健康状况仪表盘及监控解决方案，它将制造商提供的 S.M.A.R.T 指标与实际世界的故障率结合在一起。此附加组件基于 [linuxserver.io](https://www.linuxserver.io/) 上的 [docker 镜像](https://hub.docker.com/r/linuxserver/scrutiny)。
...
Web UI 可通过 <http://homeassistant:8080> 访问或通过侧边栏使用 Ingress 访问。
配置可以通过应用程序的 Web UI 完成，但以下选项除外。
它会自动挂载所有本地驱动器。

**注意**：仅在遇到问题时启用完全访问权限。在所有场景下，SMART 访问无需完全访问权限即可正常工作。
...
- **env_vars 选项**：使用附加组件的 `env_vars` 选项传递额外的环境变量（大小写名称均可）。请参阅 https://github.com/alexbelgium/hassio-addons/wiki/Add-Environment-variables-to-your-Addon-2 了解更多详情。
...
1. 在我的附加组件仓库添加到 Home Assistant 实例中（在 supervisor 附加组件商店的右上角，或者如果您已配置了我的 HA，请点击下方的按钮）
   [![Open your Home Assistant instance and show the add add-on repository dialog with a specific repository URL pre-filled.](https://my.home-assistant.io/badges/supervisor_add_addon_repository.svg)](https://my.home-assistant.io/redirect/supervisor_add_addon_repository/?repository_url=https%3A%2F%2Fgithub.com%2Falexbelgium%2Fhassio-addons)
2. 安装此附加组件。
3. 点击 `Save` 按钮以保存您的配置。
4. 将附加组件选项设置为您偏好设置
5. 启动附加组件。
6. 查看附加组件的日志以查看一切是否正常。
7. 打开 Web UI（基于 Ingress）并调整软件选项

# 在 Home Assistant 中的集成

---

与 HA 的集成可以通过配置.yaml 中的 [rest 平台](https://www.home-assistant.io/integrations/rest) 完成。

即使端口未暴露，API 也在 Home Assistant 的内部网络中可用。请使用附加组件的内部域名（`http://db21ed7f-scrutiny:8080`）从 Home Assistant 或其他附加组件中查询它。如果您需要从本地网络访问 API，请在附加组件选项中暴露端口，并将域名替换为您 Home Assistant 的 IP 地址。
...
对于详细数据，每个 HDD 可在 Scrutiny 应用程序中找到 wwn。例如：<http://db21ed7f-scrutiny-fa:8080/api/device/0x50014ee606c14537/details>
