...
我利用业余时间维护此附加组件及其他 Home Assistant 附加组件：跟踪上游更改、HA 更改，以及在真实硬件上测试需要大量时间（和一些金钱）。我使用的是 10 多个我上百个附加组件中的 5-10 个，因此我安装了测试机器（并购买一些测试服务，如 vpn），这些机器我自己不使用，用来调试和改进附加组件
...
_感谢所有人星标我的仓库！要星标它，请点击下方的图片，然后它将在右上角显示。谢谢！_

[![Stargazers repo roster for @alexbelgium/hassio-addons](https://raw.githubusercontent.com/alexbelgium/hassio-addons/master/.github/stars2.svg)](https://github.com/alexbelgium/hassio-addons/stargazers)

![下载演进图](https://raw.githubusercontent.com/alexbelgium/hassio-addons/master/tdarr/stats.png)

## 简介

[Tdarr](https://tdarr.io) 是一个分布式转码系统，用于使用 FFmpeg/HandBrake 自动化媒体库的转码/重封装管理。它确保您的文件格式、流和容器完全符合您的需求。Tdarr 支持分布式处理，允许您利用空闲硬件，通过 Windows、Linux（包括ARM）和 macOS 的 Tdarr Nodes 来实现。
...
此附加组件基于由 hurlenko 提供的 [docker 镜像](https://hub.docker.com/r/hurlenko/Tdarr)。

## 安装

此附加组件的安装非常简单，与其他任何 Hass.io 附加组件的安装方式没有不同。

1. 将我的附加组件仓库添加到您的 Home Assistant 实例中（在 supervisor addons 存储中的右上角，或者如果您已配置了我的 HA，请点击下方的按钮）
...
1. 点击 `保存` 按钮以存储配置。
1. 启动附加组件。
1. 查看附加组件日志，以确认一切是否正常。
1. 仔细根据您的偏好配置附加组件，详见官方文档。
...
| `localdisks` | str | | 本地驱动器挂载路径（例如，`sda1,sdb1,MYNAS`）。在驱动器后添加一个文件夹以只挂载该文件夹，例如 `MYNAS/public` 将只挂载该文件夹，路径为 `/mnt/MYNAS/public`。文件夹挂载需要附加组件 2026-09-19 之后版本。 |
| `networkdisks` | str | | 要挂载的 SMB 共享（例如，`//SERVER/SHARE`） |
| `cifsusername` | str | | SMB 网络共享的用户名 |
| `cifspassword` | str | | SMB 网络共享的密码 |
| `cifsdomain` | str | | SMB 网络共享的主机名 |
...
2. **添加外部节点**:
   - 在额外机器上安装 Tdarr Node
   - 指向您的 Home Assistant IP 的 `8266` 端口
   - 节点将自动注册并出现在 Web UI 中

3. **硬件加速**:
   - 此附加组件包含硬件加速支持
   - 在 Tdarr Web UI 设置中配置 GPU 转码
   - 支持的加速方案：Intel QuickSync, NVIDIA NVENC, AMD VCE

### 挂载驱动器

此附加组件支持挂载本地驱动器以及远程 SMB 共享：

- **本地驱动器**: 见 [附加组件中的挂载本地驱动器](https://github.com/alexbelgium/hassio-addons/wiki/Mounting-Local-Drives-in-Addons)
- **远程共享**: 见 [附加组件中的挂载远程共享](https://github.com/alexbelgium/hassio-addons/wiki/Mounting-remote-shares-in-Addons)
...
在 Tdarr Web UI 的 Settings > FFmpeg/HandBrake settings 下配置硬件加速。

## 支持

- 官方 Tdarr 文档: [https://docs.tdarr.io/](https://docs.tdarr.io/)
- 在 [GitHub](https://github.com/alexbelgium/hassio-addons/issues) 创建问题
- 在 [Home Assistant Community thread](https://community.home-assistant.io/t/home-assistant-addon-tdarr/282108/3) 提问
