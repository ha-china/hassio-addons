...
这是音乐助手的**BETA**版本。它包含了一些新功能和改进，这些功能在稳定版本发布前正在进行测试。
...
1. 检查应用日志（如有需要，请启用全局或按提供商级别的`debug`日志）
...
提示：如果您想在保持稳定版本的同时测试BETA版本，只需停止稳定应用并运行BETA应用。然后，切换回稳定版本就像再次停止BETA应用并启动稳定应用一样简单。两个应用不能同时激活。
...
查看[变更日志](CHANGELOG.md)以获取有关此BETA版本中新增内容的详细信息。
...
在Home Assistant中备份音乐助手应用也将包括您的音乐助手数据。请确保在更新到新版本之前始终进行备份，这样您就可以轻松地回滚到上一个版本！
