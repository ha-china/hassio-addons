...
这是音乐助手的一个**BETA**版本。它包含在稳定发布之前进行测试的新功能和改进。
...
作为一名BETA测试者，您的反馈非常有价值！请报告您遇到的问题：

### 报告之前

1. 检查插件日志（如果需要，可以全局启用或按提供程序级别启用`debug`日志记录）
...
BETA发布比稳定发布更新更频繁。一般来说，每周更新一次或几次。
...
- 版本之间可能会发生数据库迁移
- 性能优化可能仍在进行中
- 您不能从稳定版本迁移到BETA版本（反之亦然）

提示：如果您想在不保留稳定版本的情况下测试BETA版本，只需停止稳定插件并运行BETA插件。然后，回到稳定版本就像再次停止BETA插件并启动稳定插件一样简单。两个插件不能同时激活。
...
查看[CHANGELOG](CHANGELOG.md)以获取此BETA版本中新功能的详细信息。
