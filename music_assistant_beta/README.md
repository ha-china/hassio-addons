...
这是音乐助手的**BETA**版本。它包含了一些新功能和改进，这些功能在稳定版发布之前正在被测试。
...
BETA版本是功能完整版本，在成为稳定版本之前正在被测试。它们通常包括：
...
1. 检查应用程序日志（如有需要，请启用全局或按提供商级别的`debug`日志）
...
BETA版本比稳定版本更新得更频繁。通常，每周更新一次或更多。
...
提示：如果您想在保持稳定版的同时测试BETA版本，只需停止稳定应用程序并运行BETA应用程序。然后，切换回稳定版就像再次停止BETA应用程序并启动稳定应用程序一样简单。两个应用程序不能同时激活。
...
请查看[变更日志](CHANGELOG.md)以获取有关此BETA版本中新增内容的详细信息。
...
在Home Assistant中备份音乐助手应用程序也将包括您的音乐助手数据。请确保在更新到新版本之前始终进行备份，以便您可以轻松回滚到上一个版本！
