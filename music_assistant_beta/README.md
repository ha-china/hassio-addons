...
这是音乐助手的BETA版本。它包含了一些新功能和改进，这些功能在稳定版本发布前正在进行测试。
...
BETA版本是功能完整的版本，在成为稳定版本之前进行测试。它们通常包括：
...
作为BETA测试者，你的反馈非常宝贵！请报告你遇到的问题：

### 报告前的准备

1. 检查应用日志（如需要，全局或按提供者级别启用`debug`日志）
...
提示：如果你想测试BETA版本同时保留稳定版本，只需停止稳定应用并运行BETA应用。然后，回到稳定版本就像再次停止BETA应用并启动稳定应用一样简单。两个应用不能同时激活。
...
在Home Assistant中备份音乐助手应用也将包括你的音乐助手数据。请确保在更新到新版本之前始终进行备份，这样你就可以轻松地回滚到之前的版本！
...
BETA版本是稳定版本发布前的最后测试阶段。你的测试有助于确保所有用户的质量！
