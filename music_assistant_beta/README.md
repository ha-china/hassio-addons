...
这是音乐助手的**BETA**版本。它包含了一些新功能和改进，这些功能在稳定版发布之前正在进行测试。
...
BETA版本是功能完整的版本，在成为稳定版发布之前进行测试。通常包括：
...
1. 检查插件日志（如果需要，在全局或按提供者级别启用`debug`日志）
...
提示：如果您想在保留稳定版的同时测试BETA版本，只需停止稳定插件并运行BETA插件。然后，将BETA插件再次停止并启动稳定插件即可回滚到稳定版。两个插件不能同时激活。
...
在Home Assistant中对音乐助手插件进行备份也会包括您的音乐助手数据。请确保在更新到新版本之前始终进行备份，以便您可以轻松回滚到上一个版本！
...
BETA版本是稳定版发布前的最终测试阶段。您的测试有助于确保所有用户的质量！
