...
这是一个音乐助手的**BETA**版本。它包含在稳定发布前进行测试的新功能和改进。
...
1. 检查插件的日志（如果需要，可以全局启用或按提供程序级别启用`debug`日志记录）
...
BETA发布比稳定发布更新更频繁。一般来说，每周更新一到多次。
...
提示：如果你想测试BETA版本同时保留稳定版本，只需停止稳定插件并运行BETA插件。然后，要恢复到稳定版本，只需再次停止BETA插件并启动稳定版本。两个插件不能同时激活。
