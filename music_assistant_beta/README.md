...
这是音乐助手的BETA版本。它包含了一些新功能和改进，这些功能和改进在稳定版发布前正在进行测试。
...
BETA版本是功能完整的版本，在成为稳定发布版本之前正在进行测试。它们通常包括：
...
1. 检查应用日志（如需，请全局或按提供者级别启用`debug`日志）
2. 搜索[现有问题](https://github.com/music-assistant/support)
3. 如有可能，验证问题不会在稳定版本中发生
...
提示：如果您想在保留稳定版的同时测试BETA版本，只需停止稳定应用并运行BETA应用。然后，恢复到稳定版就像再次停止BETA应用并启动稳定应用一样简单。两个应用不能同时激活。
...
在Home Assistant中对音乐助手应用进行备份也将包括您的音乐助手数据。请确保在更新到新版本之前始终进行备份，以便您可以轻松地回滚到上一个版本！
...
BETA发布是稳定发布之前的最终测试阶段。您的测试有助于确保所有用户的质量！
