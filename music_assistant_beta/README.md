...
这是音乐助手的BETA版本。它包含了一些新功能和改进，这些功能在稳定版发布前正在进行测试。
...
1. 检查插件日志（如果需要，全局或按提供商级别启用`debug`日志）
...
提示：如果您想在保持稳定版的同时测试BETA版本，只需停止稳定插件并运行BETA插件。然后，重新切换回稳定版就像再次停止BETA插件并启动稳定插件一样简单。两个插件不能同时激活。
