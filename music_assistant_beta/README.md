...
这是音乐助手的BETA版本。它包含了一些新功能和改进，这些功能和改进在稳定版发布之前正在进行测试。
...
1. 检查应用日志（如需要，请全局或按提供商级别启用`debug`日志）
2. 在[现有问题](https://github.com/music-assistant/support)中搜索
3. 如可能，验证问题是否在稳定版本中发生

### 报告时包含以下内容：

- 📋 重复问题的步骤
- 📝 应用程序的全日志（或从MA的Web界面下载完整日志文件）
- 🔢 音乐助手版本（在Web UI中可见）
- 🎵 您使用的音乐提供商
- 🔊 受影响的播放器
...
提示：如果您想在保持稳定版的同时测试BETA版本，只需停止稳定应用并运行BETA应用即可。要恢复到稳定版，然后停止BETA应用并启动稳定版即可。两个应用不能同时激活。
