...
这是音乐助手的BETA版本。它包含了一些新功能和改进，这些功能在稳定版本发布之前正在被测试。
...
1. 检查应用日志（如果需要，请全局或按提供者级别启用`debug`日志）
...
提示：如果您想在保持稳定版本的同时测试BETA版本，只需停止稳定应用并运行BETA应用即可。要回滚到稳定版本，只需再次停止BETA应用并启动稳定应用。两个应用不能同时运行。
...
请查看[CHANGELOG](CHANGELOG.md)以获取有关本BETA版本中新增内容的详细信息。
...
在Home Assistant中备份音乐助手应用也会包括您的音乐助手数据。请确保在更新到新版本之前始终进行备份，以便您可以轻松回滚到上一个版本！
