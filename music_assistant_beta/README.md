...
这是一个音乐助手的**BETA**版本。它包含了在稳定发布前进行测试的新功能和改进。
...
提示：如果您希望在保留稳定版本的同时测试BETA版本，只需停止稳定插件并运行BETA插件。要恢复到稳定版本，只需再次停止BETA插件并启动稳定版本即可。两个插件不能同时激活。
