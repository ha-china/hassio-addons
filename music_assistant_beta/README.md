...
BETA 发布是功能完整的版本，在成为稳定发布之前会进行测试。它们通常包括：
...
提示：如果您希望在保留稳定版本的同时测试 BETA 版本，只需停止稳定插件并运行 BETA 插件。要切换回稳定版本，只需再次停止 BETA 插件并启动稳定版本。两个插件不能同时激活。
...
在 Home Assistant 中备份音乐助手插件也将包括您的音乐助手数据。请确保在更新到新版本之前始终进行备份，以便您可以轻松地恢复到之前的版本！
