...
这是音乐助手的**BETA**版本。它包含了一些新功能和改进，这些将在稳定版本发布前进行测试。
...
1. 检查应用日志（如有需要，在全局或按提供商级别启用`debug`日志）
...
提示：如果您想在保留稳定版本的同时测试BETA版本，只需停止稳定应用并运行BETA应用。然后，切换回稳定版本就像再次停止BETA应用并启动稳定应用一样简单。两个应用不能同时激活。
...
在Home Assistant中对音乐助手应用进行备份也会包括您的音乐助手数据。请确保在更新到新版本之前始终进行备份，以便您可以轻松地回滚到上一个版本！
