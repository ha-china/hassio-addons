...
1. 检查插件日志（如果需要，可以全局启用或针对每个提供者启用`debug`日志记录）
...
提示：如果您想在保留稳定版本的同时测试BETA版本，只需停止稳定插件并运行BETA插件。然后，要恢复到稳定版本，只需再次停止BETA插件并启动稳定插件即可。两个插件不能同时激活。
...
查看[CHANGELOG.md](CHANGELOG.md)以获取有关此BETA版本中新增内容的详细信息。
