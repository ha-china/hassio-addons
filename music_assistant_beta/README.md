...
BETA版本的更新比稳定版本更频繁。一般来说，每周更新一次或几次。
...
提示：如果你想在保留稳定版本的同时测试BETA版本，只需停止稳定插件并运行BETA插件。要恢复到稳定版本，只需再次停止BETA插件并启动稳定插件即可。两个插件不能同时激活。
...
查看[CHANGELOG.md](CHANGELOG.md)以获取此BETA版本中新增内容的详细信息。
...
在Home Assistant中备份音乐助手插件也将包括你的音乐助手数据。请确保在更新到新版本之前始终进行备份，以便可以轻松恢复到之前的版本！
...
BETA发布是稳定发布前的最终测试阶段。你的测试有助于确保所有用户的质量！
