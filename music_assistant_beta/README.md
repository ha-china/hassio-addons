...
这是音乐助手的**BETA**版本。它包含了一些新功能和改进，这些功能和改进在稳定版发布之前正在进行测试。
...
BETA版本是功能完整的版本，在成为稳定版之前进行测试。它们通常包括：
...
提示：如果您想在保留稳定版的同时测试BETA版本，只需停止稳定应用并运行BETA应用。然后，切换回稳定版就像再次停止BETA应用并启动稳定版一样简单。两个应用不能同时激活。
...
检查[变更日志](CHANGELOG.md)以获取有关本BETA版本中新功能的详细信息。
...
在Home Assistant中对音乐助手应用进行备份也会包括您的音乐助手数据。请确保在更新到新版本之前始终进行备份，以便您可以轻松地切换到上一个版本！
