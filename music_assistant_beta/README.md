...
BETA发布是功能完整的版本，在成为稳定发布之前正在接受测试。它们通常包括：
...
提示：如果您想在保留稳定版的同时测试BETA版本，只需停止稳定应用并运行BETA应用。然后，切换回稳定版就像再次停止BETA应用并启动稳定应用一样简单。两个应用不能同时激活。
...
请查看[CHANGELOG](CHANGELOG.md)以获取有关此BETA版本中新增内容的详细信息。
...
在Home Assistant中备份音乐助手应用也会包括您的音乐助手数据。请确保在更新到新版本之前始终进行备份，这样您就可以轻松地回滚到上一个版本！
