...
这是音乐助手的BETA版本。它包含了一些新功能和改进，这些功能在稳定版发布之前正在接受测试。
...
1. 检查应用日志（如果需要，请全局或按提供商级别启用`debug`日志）
...
提示：如果您想在保持稳定版的同时测试BETA版本，只需停止稳定应用并运行BETA应用即可。要回滚到稳定版，只需再次停止BETA应用并启动稳定应用。两个应用不能同时激活。
...
检查[变更日志](CHANGELOG.md)以获取此BETA版本中新增内容的详细信息。
...
在Home Assistant中备份音乐助手应用也会包括您的音乐助手数据。请确保在更新到新版本之前始终进行备份，这样您就可以轻松回滚到上一个版本！
...
访问[音乐助手组织](https://github.com/music-assistant)在GitHub上的页面以贡献。
