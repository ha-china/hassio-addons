...
我在业余时间维护这个以及其他 Home Assistant 插件：跟踪上游更改、Home Assistant 更改以及在真实硬件上进行测试都需要花费很多时间（还有一些费用）。我经常使用我超过 110 个插件中的 5-10 个，所以我安装了测试机器（并购买了某些我自身不使用的测试服务，如 VPN），以进行故障排除和改进插件。
...
WebUI 可以通过入站或访问 <http://homeassistant:PORT> 找到。端口默认是禁用的，但可以通过插件选项启用。

默认情况下，镜像基于 abc 用户，我们建议使用此用户，因为所有 init/config 都基于它。默认密码也是 abc 。如果您想更改此密码并在访问界面时需要身份验证，只需在容器内的 GUI 终端中运行 passwd 即可。然后，在访问 Web 界面时，请使用以下路径：

http://localhost:3000/?login=true

应用程序安装不会持久化，您需要通过插件选项进行操作。但是，它们的配置会持久化。

如果图形不起作用，请使用 DRINODE 功能来选择您的图形设备。
...
additional_apps: engrampa,thunderbird # 允许安装应用程序，因为它们不持久化
DRINODE: 指定自定义图形设备，默认为 /dev/dri/renderD128
DNS_servers: 8.8.8.8,1.1.1.1 # 保持空白以使用路由器的 DNS，或设置自定义 DNS 以避免在本地 DNS 广告移除器的情况下发送垃圾邮件
