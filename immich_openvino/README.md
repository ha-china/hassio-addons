...
⚠️ 该项目正处于非常活跃的开发中。请预期可能出现错误和更改。不要将其作为存储照片和视频的唯一方式！ (来自开发者)


我在业余时间维护这个和其他 Home Assistant 插件：跟踪上游更改、Home Assistant 更改以及在实际硬件上的测试都需要花费大量时间（以及一些金钱）。我经常使用我超过 110 个插件中的 5-10 个，因此我安装了测试机器（并购买了一些我本人不使用的测试服务，如 VPN），以便进行故障排除和改进插件
...
![入口](https://img.shields.io/badge/dynamic/yaml?label=入口&query=%24.ingress&url=https%3A%2F%2Fraw.githubusercontent.com%2Falexbelgium%2Fhassio-addons%2Fmaster%2Fimmich_openvino%2Fconfig.yaml)
...
![下载演变](https://raw.githubusercontent.com/alexbelgium/hassio-addons/master/immich_openvino/stats.png)

## 关于

从您的手机直接备份照片和视频的自托管解决方案，支持 OpenVINO 硬件加速。这是为使用英特尔 GPU 和 CPU 提供硬件加速的 Immich 的 OpenVINO 变体。

此插件基于 imagegenius 的 [docker 镜像](https://github.com/imagegenius/docker-immich)，其中启用了 OpenVINO 支持，以在英特尔硬件上提供增强性能。

## 硬件要求

- **英特尔硬件**：兼容的英特尔 CPU 或英特尔集成/独立 GPU
- **OpenVINO 支持**：兼容 OpenVINO 工具包的英特尔硬件
- **架构**：仅支持 AMD64（OpenVINO 支持针对英特尔 x86-64 架构进行了优化）
- **英特尔 GPU 驱动程序**：在主机系统上正确安装了英特尔 GPU 驱动程序（如果使用英特尔 GPU 加速）

## 配置

Webui 可在 `<your-ip>:8080` 找到。PostgreSQL 可以是内部的，也可以是外部的。
...
| `DISABLE_MACHINE_LEARNING` | bool | `false` | 禁用 ML 功能（不建议在 OpenVINO 变体中使用） |
...
**先决条件**：
- 用于 OpenVINO 加速的英特尔 CPU 或英特尔 GPU
- AMD64 架构（不支持 ARM）
- 安装了英特尔 GPU 驱动程序（如果使用英特尔 GPU 加速）

**步骤**：
1. 将我的插件仓库添加到您的 Home Assistant 实例中（在 supervisor 插件存储的右上角，或单击下面的按钮如果您已经配置了我的 HA）
