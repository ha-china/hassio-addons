...
使用 Cloudflared 通过远程连接您的 Home Assistant 实例，而无需开放任何端口。

## 关于

Cloudflared 通过安全的隧道将您的 Home Assistant 实例连接到 Cloudflare 上的域名或子域名。这样，您就可以向互联网暴露 Home Assistant，而无需在路由器上开放端口。此外，您还可以利用 Cloudflare Teams 及其零信任平台 further（进一步）安全您的 Home Assistant 连接。

**要使用此应用，您需要拥有一个域名（例如 example.com），并在使用 Cloudflare 作为其 DNS 条目。** 您可以在我们的 [Wiki][wiki] 中找到更多信息。

## 免责声明

请确保在使用此应用时符合 [Cloudflare 自助订阅协议][cloudflare-sssa]。
