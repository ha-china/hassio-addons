...
我在空闲时间维护此及其他 Home Assistant 附加组件：跟进上游更改、HA 更改以及在真实硬件上的测试非常耗时（且需要一些资金）。我使用大约 5-10 个我拥有的 >110 个附加组件，因此我经常安装测试机（并购买一些测试服务，如 VPN），即使我自己不使用这些服务，也是为了调试和改进附加组件。
...
_感谢所有星标了我的仓库的人！要星标它，请点击下方图片，它将显示在右上角。谢谢！_

[![Stargazers repo roster for @alexbelgium/hassio-addons](https://raw.githubusercontent.com/alexbelgium/hassio-addons/master/.github/stars2.svg)](https://github.com/alexbelgium/hassio-addons/stargazers)

![下载量变化图](https://raw.githubusercontent.com/alexbelgium/hassio-addons/master/scrutiny_fa_original/stats.png)

## 简介

---

[Scrutiny](https://github.com/AnalogJ/scrutiny) 是一个硬盘健康仪表板和监控解决方案，它将制造商提供的 S.M.A.R.T 指标与实际世界中的故障率合并。此附加组件基于 [linuxserver.io](https://www.linuxserver.io/) 提供的 [Docker 镜像](https://hub.docker.com/r/linuxserver/scrutiny)。
...
Web 界面（WebUI）可在 <http://homeassistant:8080> 或通过侧边栏的 Ingress 访问。
除了以下选项外，配置可以通过应用程序 WebUI 进行。
它会自动挂载所有本地驱动器。

**注意**：仅在遇到问题时才启用完全访问权限。在所有场景下，SMART 访问应无需完全访问权限即可正常工作。
...
| `expose_collector` | bool | | 是否在外部暴露 Collector 端口 |
...
- **env_vars 选项**：使用附加组件的 `env_vars` 选项传递额外的环境变量（大写或小写字母名称均可）。详情请见 https://github.com/alexbelgium/hassio-addons/wiki/Add-Environment-variables-to-your-Addon-2。

## 安装

---

此附加组件的安装非常直接，与其他附加组件的安装没有不同。
...
即使端口未暴露，API 也可在 Home Assistant 的内网中访问。使用附加组件的内部域名（`http://db21ed7f-scrutiny-fa-original:8080`）从 Home Assistant 或其他附加组件查询它。如果要本地网络访问 API，请在附加组件选项中暴露端口，并用您的 Home Assistant IP 地址替换域名。

有两种类型的 API 端点：

- 摘要数据：<http://db21ed7f-scrutiny-fa-original:8080/api/summary>
- 详细数据：<http://db21ed7f-scrutiny-fa-original:8080/api/device/WWN/details>

对于详细数据，可在 Scrutiny 应用程序中的每个 HDD 内找到 wwn。例如：<http://db21ed7f-scrutiny-fa-original:8080/api/device/0x50014ee606c14537/details>
...
在 github 上创建一个 issue，或在 [home assistant 线程](https://community.home-assistant.io/t/home-assistant-addon-scrutiny-smart-dashboard/295747) 上提问。
