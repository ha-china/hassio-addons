...
BirdNET-PiPy 是一个自托管的系统，它使用 BirdNET 深度学习模型来识别鸟类的叫声，并带有现代的网页仪表板用于监控检测。此插件打包了上游项目用于 Home Assistant，并支持入口。
...
- Config.yaml
使用 Filebrowser 插件中的 `/config/birdnet-pipy/config.yaml` 文件，可以使用 config.yaml 文件配置额外的变量。
