...
BirdNET-PiPy 是一个自托管的系统，它使用 BirdNET 深度学习模型来识别鸟类的叫声，并配备了一个现代的网页仪表板来监控检测。此插件将上游项目打包为适用于 Home Assistant 的格式，并支持入口。
...
1. 将我的插件仓库添加到您的 Home Assistant 实例中（在右上角的管理员插件商店中，或者如果您已配置我的 HA，请点击下面的按钮）。
...
安装后，首次启动插件。从 Home Assistant（入口）或直接在 `http://<主机>:8011`（或您配置的端口）中打开 Web UI。
...
RTSP_URL: "" # 如果 RECORDING_MODE 是 rtsp，则为必需
data_location: /config/data # BirdNET-PiPy 的持久数据位置
```

- config.yaml
在 `/config/birdnet-pipy/config.yaml` 文件中使用文件浏览器插件配置其他变量。
