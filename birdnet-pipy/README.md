...
BirdNET-PiPy 是一个自托管的系统，它使用 BirdNET 深度学习模型来识别鸟类的叫声，并带有现代的网页仪表板以监控检测。此插件将上游项目打包为适用于 Home Assistant 的版本，并支持入站。
...
- config.yaml
使用文件浏览器插件在 `/config/birdnet-pipy/config.yaml` 文件中配置额外的变量。
