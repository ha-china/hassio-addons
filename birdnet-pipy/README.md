...
BirdNET-PiPy 是一个自托管的系统，它使用 BirdNET 深度学习模型来识别鸟类声音，并配备了现代的 Web 仪表板以监控检测。此插件将上游项目打包为 Home Assistant 使用，并支持入口。
...
1. 将我的插件仓库添加到您的 Home Assistant 实例中（在右上角的监督器插件商店中，或者如果您已经配置了我的 HA，则点击下面的按钮）。
...
安装后，首次启动插件。从 Home Assistant（入口）或直接在 `http://<主机>:8011`（或您配置的端口）打开 Web UI。
