...
BirdNET-PiPy 是一个自托管的系统，它使用 BirdNET 深度学习模型来识别鸟鸣声，并配备了现代的网页仪表板以监控检测。此插件将上游项目打包到 Home Assistant 中，并支持入口。
...
- Config.yaml
使用 `/config/birdnet-pipy/config.yaml` 中的 config.yaml 文件（通过文件浏览器插件配置）可以配置额外的变量。
...
- **env_vars 选项**：使用插件的 `env_vars` 选项传递额外的环境变量（使用大写或小写名称）。有关详细信息，请参阅[向您的插件添加环境变量](https://github.com/alexbelgium/hassio-addons/wiki/Add-Environment-variables-to-your-Addon-2)。
