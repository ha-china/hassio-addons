...
这个插件的安装过程非常简单，与安装其他 Home Assistant 插件的方法相同。
...
默认情况下，上传的大小限制（例如导入操作）设置为 64MB。可以使用此选项增加该限制，例如，`100` 将是 100MB。
...
软件按“原样”提供，不提供任何形式的保证，无论是明示的还是暗示的，包括但不限于对适销性、特定用途适用性和非侵权性的保证。在任何情况下，作者或版权持有人均不对任何索赔、损害赔偿或其他责任负责，无论是由合同、侵权或其他行为引起的，也无论是由软件或其使用或其他交易引起的。
