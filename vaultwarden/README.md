...
一种用 Rust 编写的 Bitwarden 服务器 API 的替代实现，与上游 Bitwarden 客户端*兼容，非常适合自托管部署，因为在运行官方资源密集型服务可能不太理想的情况下，你可以选择运行这个服务。

此版本、官方 homeassistant 插件和 Alex Belgium 的插件之间的区别在于，它将数据存储在 /addons_config 中，这使得在意外卸载或进行不良升级时更容易移动它们。你必须确保使用 argon 加密的密码，这应该现在是默认的。此外，内置的 homeassistant 插件通常不会更新（即使多次请求也是如此）。此插件也仅使用官方的 docker 镜像，没有任何更改，而其他插件会编辑镜像并添加额外的内容。
...
如果你有一个现有的 vaultwarden 安装（默认插件或 alexbelgium 的插件），
...
1. 你可以通过停止容器并编辑 `/addon-configs/2effc9b9_vaultwarden/config.json` 手动配置许多参数
...
4. 由于这个文件是可访问的，我猜任何人都可以这样做，所以要小心。如果你有访问你的 homeassistant 机器的权限，这也可以在容器内部完成，所以实际上并没有更安全
