...
Bedrock Server Manager 是一款全面的 Python 服务器，旨在轻松安装、管理和维护 Minecraft Bedrock 专用服务器。
...
此扩展的安装过程非常简单，与安装任何其他 Hass.io 扩展没有太大区别。
...
1. 设置在 `/addon_configs/2effc9b9_bedrockservermanager` 中。
...
Ingress 只部分工作。它不会显示实时“监控标签”或其他 WebSocket 调用。但是，基本的行政管理是可用的。您必须访问 ip:端口 以获得完整功能
