...
一套基于浏览器的工具，供D&D 5e的玩家和DM使用。从5etools GitHub下载发布图像。jdeath的仓库中没有托管或发布任何图像或内容。由于Home Assistant插件创建者不使用它，因此不提供支持。自托管的图像可能落后于5etools网站的一个版本。图像大小为4 GB，因此安装需要很长时间，请耐心等待。
