...
该插件使用每天凌晨 02:00 UTC 自动构建的 ESPHome 版本，用于测试开发中的组件。请参考下方的 `esphome_fork` 配置以正确配置插件。一旦更新了配置，请确保重新构建镜像。
...
如果设置为 `true`，这将启用流模式，使 ESPHome 隐藏所有潜在的私人信息。例如 WiFi (B)SSID（可能用于查找你的位置）、用户名等。
请注意，你需要使用 YAML 文件中的 `!secret` 标签来阻止这些信息在编辑和验证时显示。