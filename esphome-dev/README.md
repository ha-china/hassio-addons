...
此插件允许您通过 Home Assistant **无需任何编程经验** 将配置写入并直接将您的微控制器转换为智能家居设备。您只需要编写 YAML 配置文件；其余的（空中更新、编译）都由 ESPHome 处理。
...
使用 ESPHome，您可以从几行 YAML 直接跳转到自定义固件。例如，要包含 [DHT22][dht22] 温湿传感器的配置，您只需在配置文件中包含 8 行 YAML 即可：
...
[版本]: https://next.esphome.io/changelog/index.html
