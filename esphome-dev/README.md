...
此插件允许您通过 Home Assistant **无需编程经验** 直接编写配置并使您的微控制器成为智能家居设备。您只需编写 YAML 配置文件；其余的（空中更新、编译）都由 ESPHome 处理。
...
使用 ESPHome，您可以从几行 YAML 配置直接跳到定制的固件。例如，要包含 [DHT22][dht22] 温度和湿度传感器，您只需在配置文件中包含 8 行 YAML：
