...
_"外观和气味都像 Overseerr，但做法相反。"_

Maintainerr 是一个用于 Plex、Jellyfin 或 Emby 生态系统的基于规则的媒体管理工具。它可以根据可配置的规则（观看状态、年龄、评分等）创建智能合集，并且可选地删除未观看的内容以保持库的整洁。
...
|----------|---------|--------|
| `UI_PORT` | `6246` | 更改监听端口 |
| `BASE_PATH` | _(空值)_ | 在 URL 子路径下提供服务 |

## 数据

持久化数据（数据库、配置）存储在 HA 插件配置目录中，并在插件更新和重新安装时得以保留。
