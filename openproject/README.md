...
我利用业余时间维护这个Home Assistant插件和其他插件：跟进上游更改、HA更改以及在真实硬件上测试需要大量时间（并且需要一些金钱）。我大约使用我超过110个插件中的5-10个，因此我安装了测试机器（并购买了一些我自己不使用的测试服务，例如VPN），以便调试和改进插件。
...
使用插件的`env_vars`选项来传递额外的环境变量（大小写名称）。请参阅 https://github.com/alexbelgium/hassio-addons/wiki/Add-Environment-variables-to-your-Addon-2 了解详细信息。

填写默认插件选项以启动插件。务必特别配置主机名，使用您的homeassistant IP + 插件暴露端口。对于其他选项，请使用config.yaml系统：https://github.com/alexbelgium/hassio-addons/wiki/Addons-feature:-add-env-variables
