...
我在业余时间维护这个和其他 Home Assistant 扩展：跟踪上游更改、HA 更改以及在真实硬件上进行测试需要花费大量时间（以及一些金钱）。我经常使用大约 5-10 个我 >110 个扩展，所以我安装了测试机器（并购买了某些测试服务，如 vpn），这些服务我自己并不使用，以便进行故障排除和改进扩展。
...
_感谢每一位给我的仓库点赞的人！要给仓库点赞，请点击下面的图片，然后它就会出现在右上角。谢谢！_
...
[wger](https://github.com/wger-project/wger) 锻炼管理器是一个免费的开源网络应用程序，可以帮助您管理个人锻炼、体重和饮食计划，也可以用作简单的健身房管理工具。它还提供了一个 REST API，便于与其他项目和工具集成。
...
- 启动扩展。稍等片刻，检查日志中是否有任何错误。初始启动可能需要长达 15 分钟！
- 打开您的域名.com:9927（默认主机映射为扩展端口 `80/tcp`，如 `webui` 提示所示）。
- 默认
  - 用户名：`admin`
  - 密码：`adminadmin`
...
可以通过在 config.yaml 中添加它们来设置作为 ENV 变量的额外变量，该 config.yaml 位于您在扩展选项中定义的位置，请参阅此指南：https://github.com/alexbelgium/hassio-addons/wiki/Addons-feature:-add-env-variables

ENV 变量的完整列表可在此处查看：不可用
