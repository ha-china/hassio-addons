...
Homebox 是为家庭用户构建的库存和组织系统！它专注于简洁性和易用性，是您家庭库存、组织和管理的完美解决方案。在开发这个项目时，我尽量遵循以下原则：

- _简单_ - Homebox 被设计得简单易用。无需复杂的设置或配置。可以使用单个 Docker 容器，或者编译成您选择的平台的二进制文件自行部署。
- _速度极快_ - Homebox 使用 Go 语言编写，这使得它非常快速且部署资源最小。一般来说，整个容器的空闲内存使用量不到 50MB。
