...
![ShieldDNS Logo](https://raw.githubusercontent.com/FaserF/hassio-addons/master/ShieldDNS/logo.png) width="100" alt="Logo" />

[![打开您的 Home Assistant 实例并显示应用仪表板](https://my.home-assistant.io/badges/supervisor_addon.svg)](https://my.home-assistant.io/redirect/supervisor_addon/?addon=c1e285b7_ShieldDNS)
[![Home Assistant 应用](https://img.shields.io/badge/home%20assistant-app-blue.svg)](https://www.home-assistant.io/apps/)
[![Docker 镜像](https://img.shields.io/badge/docker-2.4.0-blue.svg?logo=docker&style=flat-square)](https://github.com/FaserF/hassio-addons/pkgs/container/hassio-addons-shielddns)
...
- 🔒 **全面双支持**：原生支持 **DNS-over-TLS (DoT)**（端口 853）和 **DNS-over-HTTPS (DoH)**（端口 443），以高效率处理。
- 📊 **管理仪表板**：高级网页界面用于实时统计和配置。
- 🛡️ **DNS 过滤**：集成的引擎用于阻止列表，具有自动更新和去重功能。
- ⚡ **高性能**：基于 CoreDNS 和 Go，以最大效率构建。
...
- **初始设置**：首次访问时，您将被提示设置一个 12 位的管理员密码。
- **过滤**：直接从 UI 管理您的阻止列表（AdGuard、Pi-hole 等）。
- **统计**：实时查看总查询、被阻止请求和阻止比率。

## 📱 客户端配置

### DoT (DNS-over-TLS) - 端口 853

- **Android**：转到 **设置 > 网络 > 私有 DNS** 并输入 `dns.example.com`。
...
由于您正在将 DNS 服务器暴露给公众，您应该确保其安全性：

1. **使用 WAF**：在 DoH 端点前面放置反向代理或 Cloudflare Tunnel。
...
| 协议 | 端口 | TCP 端口 | 描述 | 支持 |
| :--- | :--- | :------- | :--- | :--- |
| **DoT** | `853` | 专用安全 DNS 端口 | **原生**（Android 私有 DNS） | 是 |
| **DoH** | `443` | 标准 HTTPS 网页端口 | **原生**（Windows 11、iOS、浏览器） | 是 |

## 🏠 Home Assistant 插件

ShieldDNS 可作为官方 Home Assistant 插件提供，具有完整的 **入口** 支持用于管理仪表板。
