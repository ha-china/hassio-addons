...
n8n 是一款可扩展的工作流自动化工具。采用公平的代码分发模式，n8n 将始终具有可见的源代码，可供自托管，并允许您添加自己的自定义函数、逻辑和应用程序。n8n 的基于节点的架构使其非常灵活，能够让您将任何东西连接到任何东西。

功能尚未测试，但插件运行正常

_感谢所有给我的仓库点星的人！要给仓库点星，请点击下面的图片，然后它将显示在右上角。谢谢！_
...
1. 将我的 Hass.io 扩展插件仓库 [repository] 添加到您的 Hass.io 实例中。
1. 点击 `保存` 按钮以存储您的配置。
1. 启动插件。
1. 插件会失败，这是正常的
...
您可以选择将插件设置为使用环境文件。注意使用 '/home/node' 作为基础路径，它将映射到 /addon_configs/2effc9b9_n8n 
