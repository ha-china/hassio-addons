...
- 🎶 **统一库**：将来自不同来源的所有音乐集中在一个地方
- 🎯 **智能播放**：无缝播放、交叉渐变和音频归一化
...
**推荐**：仅在需要解决任何问题时使用 `debug` 级别。最好将全局设置保持为仅 `info`。
...
安装插件后（或安装您网络中的任何音乐助手服务器），Home Assistant 应自动检测音乐助手服务器。在设备和服务页面上，您应该会看到为发现的服务器提供的卡片，以便简单设置集成。

**注意**：插件提供音乐助手服务器，而集成提供 Home Assistant 实体和自动化功能。如果您只想使用网页界面，则无需集成，插件也能完美工作。
...
这是 **稳定** 渠道。更新是在彻底测试后发布的，并推荐用于日常使用。
...
此插件使用音乐助手的稳定版本。对于最新功能，请考虑使用 BETA 或 NIGHTLY 版本（使用风险自负）。
...
因此，在 Home Assistant 中备份音乐助手插件也将包含您的音乐助手数据。请确保在更新到新版本之前始终进行备份，以便您可以轻松地恢复到以前的版本！
...
访问 GitHub 上的 [音乐助手组织](https://github.com/music-assistant) 以贡献。

## 许可证

音乐助手在 Apache License 2.0 下许可。
## 📱 关注我
