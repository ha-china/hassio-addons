...
音乐助手是一个免费、开源的音乐库管理器，它连接到您的流媒体服务和广泛的连接扬声器。将您的Home Assistant实例变成您自己的个人音乐流媒体中心！
...
**建议**：考虑仅在故障排除时使用`debug`级别。最好将全局设置保持在`info`。
...
为了进行高级自动化和控制，您可以可选地安装Home Assistant中的**音乐助手集成**。此集成允许您：
...
在您安装应用（或网络中的任何音乐助手服务器）后，Home Assistant应自动检测音乐助手服务器。在设备与服务页面，您应该会看到一个用于设置集成的卡片。

**注意**：应用提供了音乐助手服务器，而集成提供了Home Assistant实体和自动化功能。如果您只想使用网络界面，则无需集成。
...
1. 检查应用日志（可在Home Assistant应用页面中找到）
...
因此，备份Home Assistant中的音乐助手应用也将包括您的音乐助手数据。请确保在更新到新版本之前始终进行备份，以便您可以轻松地回滚到上一个版本！
