...
[Changedetection.io](https://github.com/dgtlmoon/changedetection.io) 提供免费、开源的网页监控、通知和变化检测服务。

此插件基于 linuxserver.io 的 [docker 镜像](https://github.com/linuxserver/docker-changedetection.io)。

## 配置

使用插件的 `env_vars` 选项来传递额外的环境变量（名称可以是大小写）。有关详细信息，请参阅 https://github.com/alexbelgium/hassio-addons/wiki/Add-Environment-variables-to-your-Addon-2。
...
4. 填写侧边栏项目的标题、一个图标（建议：`mdi:vector-difference`），并为该面板提供一个**相对 URL**（例如 `change-detection`）。最后，确认它。
