...
我利用业余时间维护此及其他 Home Assistant 插件：跟踪上游更改、HA 更改以及在真实硬件上进行测试需要耗费大量时间（以及一些金钱）。我会定期使用大约 5-10 个我拥有的 >110 个插件，因此我会安装测试机器（并购买一些测试服务，如 vpn），而不用于我自己的测试和工具，以便解决和完善插件。

如果此插件为您节省了时间或使您的设置更简单，我会非常感谢您的支持！
...
_感谢所有人给我的仓库星标！要星标它，请点击下方的图片，然后它将显示在右上角。谢谢！_
...
使用插件的 `env_vars` 选项传递额外的环境变量（名称可以为大写或小写）。详情请见 https://github.com/alexbelgium/hassio-addons/wiki/Add-Environment-variables-to-your-Addon-2。

### 主应用

Web UI 可在 `<your-ip>:5000` 访问，也可通过插件页面或侧边栏中的 Ingress 访问。
...
3. 选择 <kbd>网页</kbd> 选项，并将来自插件页面的 Web UI URL 粘贴进去。
4. 输入侧边栏项的标题，选择一个图标（建议：`mdi:vector-difference`），并为该面板填写一个 **相对 URL**（例如 `change-detection`）。最后，确认它。
...
安装并启动 Browserless Chrome 插件，然后使用 `PLAYWRIGHT_DRIVER_URL` 选项连接它。此选项必须填写 Browserless Chrome 的 URL："ws://db21ed7f-browserless-chrome:3000/chromium?headless=true&stealth=true&blockAds=true"
...
此插件的安装非常简单，与其他 Hass.io 插件的安装方式没有区别。

1. 将我的插件仓库添加到您的 home assistant 实例中（在 supervisor addons 存储器的右上角，或者如果您已配置好的 HA 则点击下方按钮）
   [![Open your Home Assistant instance and show the add add-on repository dialog with a specific repository URL pre-filled.](https://my.home-assistant.io/badges/supervisor_add_addon_repository.svg)](https://my.home-assistant.io/redirect/supervisor_add_addon_repository/?repository_url=https%3A%2F%2Fgithub.com%2Falexbelgium%2Fhassio-addons)
1. 安装此插件。
1. 点击 `Save` 按钮以保存您的配置。
1. 启动插件。
1. 检查插件日志以查看一切是否顺利。
1. 仔细根据您的偏好配置插件，有关您偏好的官方文档，请参阅官方文档。
