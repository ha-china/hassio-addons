...
Web 界面可以在 `<你的IP>:5000` 找到，也可以从插件页面访问。
...
3. 选择 <kbd>Webpage</kbd> 选项，并粘贴您从插件页面获得的 Web UI URL。
...
在 HA 中，使用文件编辑插件（或 Filebrowser）并编辑 Changedetection.io 配置文件 `/homeassistant/addons_config/changedetection.io/config.yaml`。
...
1. 启动插件。
1. 检查插件的日志以查看是否一切顺利。
1. 仔细配置插件以满足您的需求，请查看官方文档以获取详细信息。
