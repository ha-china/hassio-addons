...
此部分定义了在 MariaDB 中的创建用户定义。[Create User][createuser] 文档。
...
要授予此用户的权限列表，从 [Grant][grant] 如 `SELECT` 和 `CREATE`。
如果省略，则向用户授予 `ALL PRIVILEGES`。限制 Home Assistant 使用用户的权限是不推荐的，但如果您希望允许其他应用程序查看 recorder 数据，则应创建一个仅具有数据库只读访问权限的用户。

### 选项：`mariadb_server_args`（可选）

一些用户在 Home Assistant 大型数据库的架构更新期间遇到了 [错误][migration-issues]。
如果您的系统有足够的 RAM，定义推荐参数可能会有所帮助。
...
- [Home Assistant Discord Chat Server][discord]。
