...
针对版本提取、比较和升级决策逻辑的纯 bash 单元测试。

**要求：**
- 基本bash工具（`sort`、`sed`、`tr`）
- 无需编译器
...
3. **触发升级检查：**
   ...
   - 检查日志中的升级消息：
     ```
     [INFO] 已将 srat-cli 从 2025.12.0 升级到 2025.12.1
     ```
...
**"gcc 不可用"**
- 集成测试将优雅地跳过
- 如果您需要完整测试，请安装 gcc：`apt-get install gcc`

**版本提取返回空**
- 验证二进制文件是否有 `.note.metadata` 部分：`objdump -s --section .note.metadata <binary>`
- 检查该部分是否包含带有 "version" 字段的 JSON
