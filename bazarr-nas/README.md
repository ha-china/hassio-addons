...
我在闲暇时间维护此以及其他 Home Assistant 附加组件：跟踪上游更改、HA 更改并在真实硬件上进行测试需要花费很多时间（以及一些资金）。我使用的附加组件大约有 5-10 个，所以我定期安装测试机（并且购买一些我本人不使用的服务，例如 VPN）来排查问题和改进附加组件。

如果您使用此附加组件节省了时间或让配置更简单，我将非常感激您的支持！
...
![版本](https://img.shields.io/badge/dynamic/yaml?label=版本&query=%24.version&url=https%3A%2F%2Fraw.githubusercontent.com%2Falexbelgium%2Fhassio-addons%2Fmaster%2Fbazarr%2Fconfig.yaml)
...
![架构](https://img.shields.io/badge/dynamic/yaml?color=success&label=架构&query=%24.arch&url=https%3A%2F%2Fraw.githubusercontent.com%2Falexbelgium%2Fhassio-addons%2Fmaster%2Fbazarr%2Fconfig.yaml)
...
[donation-badge]: https://img.shields.io/badge/给作者买杯咖啡-%23d32f2f?logo=buy-me-a-coffee&style=flat&logoColor=white
[paypal-badge]: https://img.shields.io/badge/通过 PayPal 捐赠-0070BA?logo=paypal&style=flat&logoColor=white

_感谢所有为我仓库点 starred 的人！要 star 它，请点击下方的图片，它将被显示在右上角。非常感谢您的支持！_

[![@alexbelgium/hassio-addons 星号存储仓库名单](https://raw.githubusercontent.com/alexbelgium/hassio-addons/master/.github/stars2.svg)](https://github.com/alexbelgium/hassio-addons/stargazers)

![下载量趋势](https://raw.githubusercontent.com/alexbelgium/hassio-addons/master/bazarr/stats.png)

## 简介

---

[Bazarr](https://www.bazarr.media/) 是 Sonarr 和 Radarr 的配套应用程序，根据您的需求管理并下载字幕。此附加组件基于 Docker 镜像 https://github.com/linuxserver/docker-bazarr。

## 配置

使用附加组件的 `env_vars` 选项传递额外的环境变量（大写或小写名称均可）。详情参见 https://github.com/alexbelgium/hassio-addons/wiki/Add-Environment-variables-to-your-Addon-2。

WebUI 可在 <http://homeassistant:PORT> 或通过旁边栏中的 Ingress 访问。

配置可以通过应用程序的 WebUI 进行，除了以下选项外。
...
- `ingress_noauth` - 默认模式，禁用认证以启用无缝的 Ingress 集成。
- `noingress_auth` - 禁用 Ingress 并允许外部 URL，启用认证。
...
1. 将我的附加组件仓库添加到您的 home assistant 实例中（在 supervisor 附加组件商店的右上角，或者如果您已配置了我的 HA，请点击下方的按钮）
   [![打开您的 Home Assistant 实例并显示带有预填充特定仓库 URL 的“添加附加组件仓库”对话框。](https://my.home-assistant.io/badges/supervisor_add_addon_repository.svg)](https://my.home-assistant.io/redirect/supervisor_add_addon_repository/?repository_url=https%3A%2F%2Fgithub.com%2Falexbelgium%2Fhassio-addons)
2. 安装此附加组件。
3. 点击 `保存` 按钮以存储配置。
4. 将附加组件选项设置为您的偏好设置。
5. 启动附加组件。
