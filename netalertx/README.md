...
我在业余时间维护这个和其他 Home Assistant 插件：跟进上游更改、HA 更改以及在真实硬件上进行测试需要花费很多时间（还有一些钱）。我经常使用大约 5-10 个我 >110 个插件中的几个，所以我安装了测试机器（并购买了我不使用的某些测试服务，如 vpn），以便进行故障排除和改进插件。
...
1. 将我的插件存储库添加到您的 Home Assistant 实例中（在管理员的插件存储库中右上角，或点击下面的按钮如果您已经配置了我的 HA）
...
1. 启动插件。
1. 检查插件的日志以查看是否一切顺利。
1. 仔细配置插件以满足您的需求，有关详细信息，请参阅官方文档。

## 配置

1. 在首次运行时，如果没有可用，应用程序会生成默认的 `app.conf` 和 `app.db` 文件。
1. 最推荐的方式是通过 UI 中的设置部分管理配置，如果 UI 无法访问，您可以直接在 `/config/config/` 文件夹中修改 `app.conf`。
1. 您必须指定要扫描哪个（些）网络。这是通过输入主机可访问的子网来完成的。如果您使用默认的 `ARPSCAN` 插件，您必须在 `SCAN_SUBNETS` 设置中指定至少一个有效的子网和接口。有关如何设置多个 SUBNETS、VLANs 及其限制的信息，请参阅 [SUBNETS 文档](https://github.com/jokob-sk/NetAlertX/blob/main/docs/SUBNETS.md)，有关故障排除和更高级的场景，请参阅该文档。
1. 有关如何通过 MQTT 插件将设备添加到您的 [Home Assistant 实例中的说明](https://github.com/jokob-sk/NetAlertX/blob/main/docs/HOME_ASSISTANT.md)
