...
我在业余时间维护这个以及其他 Home Assistant 插件：跟进上游更改、HA 更改以及在真实硬件上进行测试需要花费很多时间（以及一些金钱）。我经常使用我超过 110 个插件中的 5-10 个，所以我安装了测试机器（并购买了某些测试服务，如 vpn），这些服务我自己并不使用，以便进行故障排除和改进插件。
...
网络存在性和入侵检测器。扫描连接到您网络上的设备，并在发现新的未知设备时发出警报。
...
1. 如果不可用，应用程序在第一次运行时会在 `/config/config/` 文件夹中生成默认的 `app.conf` 和 `app.db` 文件。
1. 最好通过 UI 中的设置部分管理配置，如果 UI 不可访问，您可以直接在 `/config/config/` 文件夹中修改 `app.conf`。
1. 您必须指定要扫描哪些网络。这是通过输入从主机可访问的子网来完成的。如果您使用默认的 `ARPSCAN` 插件，您必须在 `SCAN_SUBNETS` 设置中指定至少一个有效的子网和接口。有关如何设置多个 SUBNETS、VLANs 以及限制的说明，请参阅 [如何设置多个 SUBNETS、VLANs 以及限制](https://github.com/jokob-sk/NetAlertX/blob/main/docs/SUBNETS.md)，以及有关故障排除和更高级场景的说明。
