...
网络存在性和入侵检测器。扫描连接到您的网络上的设备，并在发现新的未知设备时发出警报。
...
1. 您必须指定应扫描哪些网络。这是通过输入从主机可访问的子网来完成的。如果您使用默认的 `ARPSCAN` 插件，您必须在 `SCAN_SUBNETS` 设置中指定至少一个有效的子网和接口。有关如何设置多个子网、VLAN 以及限制的说明，请参阅 [如何设置多个子网、VLAN 及其限制](https://github.com/jokob-sk/NetAlertX/blob/main/docs/SUBNETS.md)，以及有关故障排除和更高级场景的说明。
1. 有关如何通过 MQTT 插件将设备添加到您的 [Home Assistant 实例](https://github.com/jokob-sk/NetAlertX/blob/main/docs/HOME_ASSISTANT.md) 的说明。
1. 按照以下说明进行备份 [备份文档](https://github.com/jokob-sk/NetAlertX/blob/main/docs/BACKUPS.md)。
