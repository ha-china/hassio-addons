...
我在业余时间维护这个以及其他 Home Assistant 插件：跟进上游变更、Home Assistant 变更以及在真实硬件上进行测试都需要花费大量时间（以及一些金钱）。我经常使用 5-10 个我超过 110 个插件中的部分，所以我安装了测试机器（并购买了一些我自己不使用的测试服务，如 vpn），以进行故障排除和改进插件。
...
WebUI 可在 <http://homeassistant:PORT> 或通过侧边栏使用入站找到。
