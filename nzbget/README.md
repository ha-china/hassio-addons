...
我利用业余时间维护这个和其他的Home Assistant插件：跟上上游的变更、Home Assistant的变更以及在真实硬件上测试需要花费大量时间（和一些金钱）。我大约使用我超过110个插件中的5-10个，因此我安装了一些测试机器（和一些我自身不使用的测试服务，如VPN）来调试和改进插件。
...
[nzbget](http://nzbget.net/) 是一个usenet下载器，用C++编写，设计时注重性能，以使用最少的系统资源来达到最大下载速度。
这个插件基于这个docker镜像 https://github.com/linuxserver/docker-nzbget
...
- **env_vars 选项**：使用插件的 `env_vars` 选项来传递额外的环境变量（大小写名称都可以）。参见 https://github.com/alexbelgium/hassio-addons/wiki/Add-Environment-variables-to-your-Addon-2 获取详细信息。
