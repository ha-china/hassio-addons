...
我在业余时间维护这个和其他 Home Assistant 插件：跟踪上游变更、Home Assistant 变更以及在实际硬件上的测试都需要花费很多时间（以及一些金钱）。我经常使用大约 5-10 个我 >110 个插件，所以我安装了测试机器（并购买了些我不使用的测试服务，如 vpn），来排查问题和改进插件。
...
[jackett](https://github.com/jackett/jackett) - 一个用于与 Couchpotato 一起工作，处理电影的 jackett 分支。
...
Webui 可以在配置的端口找到，或者通过侧边栏使用入口访问。
除了以下选项外，配置可以通过应用的 webUI 完成。
...
1. 将我的插件仓库添加到您的 Home Assistant 实例中（在监督器的插件存储中右上角，或者如果您已配置我的 HA，请点击下面的按钮）
