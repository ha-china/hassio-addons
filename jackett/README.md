...
我在业余时间维护这个以及其他 Home Assistant 扩展：跟进上游变更、HA 变更以及在实际硬件上进行测试都需要花费大量时间（以及一些金钱）。我经常使用大约 5-10 个我 >110 个扩展，所以我安装了测试机器（并购买了一些我自己不使用的测试服务，如 VPN）来调试和改进扩展。
...
[jackett](https://github.com/jackett/jackett) - jackett 的一个分支，用于与 Couchpotato 等电影一起工作。

此扩展基于 [docker 镜像](https://github.com/linuxserver/docker-jackett)。
...
Webui 可以在配置的端口上找到，或者通过侧边栏使用 Ingress。
...
1. 将我的扩展仓库添加到您的 Home Assistant 实例中（在监督器的扩展存储中右上角，或者如果您已配置我的 HA，请点击下面的按钮）
...
1. 启动扩展。
1. 检查扩展的日志以查看是否一切顺利。
1. 仔细配置扩展以满足您的需求，有关详细信息，请参阅官方文档。
