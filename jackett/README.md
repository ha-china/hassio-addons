...
我在业余时间维护这个和其他 Home Assistant 插件：跟踪上游变更、Home Assistant 变更以及在真实硬件上进行测试需要花费大量时间（以及一些金钱）。我经常使用 5-10 个我超过 110 个插件中的几个，所以我安装了测试机器（并购买了一些我本人不使用的测试服务，如 VPN），以便进行故障排除和改进插件。
...
[jackett](https://github.com/jackett/jackett) - 一个 jackett 的分支，用于与 Couchpotato 等电影一起工作。

此插件基于 [docker 镜像](https://github.com/linuxserver/docker-jackett)。
...
Webui 可在配置的端口或通过侧边栏使用 Ingress 访问。
...
1. 将我的插件仓库添加到您的 Home Assistant 实例中（在监督器插件商店的右上角，或单击下面的按钮如果您已配置我的 HA）
