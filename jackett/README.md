# Home Assistant 扩展：Jackett


我在业余时间维护这个以及其他 Home Assistant 扩展：跟上上游的变化、HA 的变化以及在实际硬件上进行测试需要花费大量的时间和一些金钱。我经常使用大约 5-10 个我超过 110 个扩展，所以我安装了测试机器（并购买了某些测试服务，例如 vpn），这些服务我自己并不使用，以进行故障排除和改进扩展。

如果这个扩展节省了你的时间或使你的设置更简单，我会非常感激你的支持！
...
![入站](https://img.shields.io/badge/dynamic/yaml?label=Ingress&query=%24.ingress&url=https%3A%2F%2Fraw.githubusercontent.com%2Falexbelgium%2Fhassio-addons%2Fmaster%2Fjackett%2Fconfig.yaml)
...
_感谢每一位给我的仓库加星的用户！要加星，请点击下面的图片，然后它就会出现在右上角。谢谢！_
...
[jackett](https://github.com/jackett/jackett) - jackett 的分支，用于与 Couchpotato 之类的电影一起使用。

此扩展基于 [docker image](https://github.com/linuxserver/docker-jackett) 来自 linuxserver.io。
...
Webui 可以在配置的端口或通过侧边栏使用 Ingress 访问。
可以通过应用程序的 WebUI 进行配置，但以下选项除外。
...
1. 将我的扩展仓库添加到您的 Home Assistant 实例中（在监督者扩展存储的右上角，或单击下面的按钮如果您已经配置了我的 HA）
