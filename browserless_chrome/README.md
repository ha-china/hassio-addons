...
我利用业余时间维护此及其他 Home Assistant 附加组件：跟进上游更改、Home Assistant 更改以及在实际硬件上的测试耗费了大量时间（以及一些费用）。我运行的 110 多个附加组件中大约有 5-10 个，我经常安装测试机器（并购买一些我自己不使用的测试服务，如 vpn），以排查问题并改进附加组件。

如果这个附加组件为您节省了时间或使您的设置变得更简单，您的支持将不胜感激！
...
_感谢所有为我仓库点星的人！点击下图即可点星，它将显示在右上角。谢谢！_
...
[Browserless chrome](https://github.com/browserless/chrome) 是一个允许远程客户端连接、控制并执行无头任务的 Web 服务。此附加组件基于 docker 镜像 https://hub.docker.com/r/browserless/chrome/。

## 配置
---

WebUI 可在 <http://homeassistant:PORT> 访问。配置可通过 app WebUI 完成，除了以下选项：
...
- **env_vars 选项**：使用附加组件的 `env_vars` 选项传递额外的环境变量（名称可大写或小写）。详见 https://github.com/alexbelgium/hassio-addons/wiki/Add-Environment-variables-to-your-Addon-2

## 安装

---

此附加组件的安装非常 straightforward，与其他附加组件安装没有差异。

1. 将我的附加组件库添加到您的 Home Assistant 实例中（在 supervisor 附加组件商店右上角，或如果您已配置了我的 HA，则点击下方按钮）
