...
我在业余时间维护这个以及其他 Home Assistant 插件：跟进上游更改、Home Assistant 更改以及在实际硬件上的测试需要花费大量时间（以及一些金钱）。我经常使用 5-10 个我超过 110 个插件中的几个，因此我安装了测试机器（并购买了一些我自己不使用的测试服务，如 VPN）来调试和改进插件。

如果这个插件能为您节省时间或使您的设置更容易，我将非常感谢您的支持！
...
WebUI 可在 `<你的 IP>:8096` 上找到，或通过入口在 Home Assistant 内部访问。

```yaml
PGID: user
GPID: user
TZ: timezone
localdisks: sda1 #将你的驱动器的硬件名称放在这里以挂载，用逗号分隔，或使用其标签。例如：sda1, sdb1, MYNAS...
...
1. 将我的插件存储库添加到您的 Home Assistant 实例中（在监督器插件的右上角添加插件存储库，或点击下面的按钮如果您已经配置了我的 HA）
