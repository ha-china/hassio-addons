...
我在业余时间维护这个以及其他 Home Assistant 插件：跟踪上游更改、Home Assistant 更改以及在真实硬件上进行测试都需要花费大量时间（以及一些金钱）。我经常使用 5-10 个我超过 110 个插件中的几个，所以我安装了测试机器（并购买了一些我本人不使用的测试服务，如 VPN），以便进行故障排除和改进插件。
...
Webui 可在 `<您的 IP>:8096` 找到，或通过入口在 Home Assistant 内部访问。

```yaml
PGID: user
GPID: user
TZ: timezone
localdisks: sda1 #请将您的驱动器硬件名称放入此处以挂载，用逗号分隔，或使用其标签。例如：sda1, sdb1, MYNAS...
