...
[Inadyn](https://github.com/troglobit/inadyn)，或 In-a-Dyn，是一个小型且简单的动态DNS、DDNS客户端，支持HTTPS。在许多GNU/Linux发行版中常见，常用于成品路由器和互联网网关，以自动化保持您的互联网名称与您的公共IP地址¹同步的任务。它也可以用于具有冗余（备用）互联网连接的安装。
...
1. 启动插件。
1. 检查插件的日志以查看一切是否正常。
1. 仔细配置插件以满足您的需求，请参阅官方文档以了解如何配置。
...
| 选项 | 类型 | 描述 |
|------|------|------|
| `provider` | 字符串 | 提供程序名称或自定义标识符 |
| `custom_provider` | 布尔 | 是否为自定义提供程序 |
...
您不需要填写所有这些内容，只需使用必要的部分。一个典型的示例看起来像这样：
...
ddns_path中的令牌在`inadyn.conf(5)`手册页中有详细说明。
