...
| `app_base_url`           | string  | (自动)  | 应用的基本 URL（用于电子邮件/链接）。如果为空，则从 Ingress 自动检测 |
| `marketing_page_enabled` | boolean | `false` | 启用营销页面                                                    |

> **注意**: OAuth 提供商和 SMTP 设置在首次登录后在**管理面板**中配置，而不是在这里。
...
> - 所有设置和配置
>
> 这无法撤销！重置后，您必须手动禁用此选项，否则每次重启时您的数据将被清除。
...
- **替代方案**: 点击“打开 Web UI”或访问 `http://homeassistant.local:8099`
