...
| 选项                   | 类型    | 默认 | 描述                                                                  |
| :----------------------- | :------ | :------ | :------------------------------------------------------------------- |
| `log_level`              | select  | `info`  | 日志详细程度：trace, debug, info, warning, error, fatal             |
| `test_mode`              | boolean | `false` | 启用测试模式，使用虚拟用户数据进行测试                                |
| `app_base_url`           | string  | (auto)  | 应用的基本 URL（用于邮件/链接）。如果为空，则自动从 Ingress 检测 |
| `marketing_page_enabled` | boolean | `false` | 启用营销页面                                                        |

> **注意**: OAuth 提供者和 SMTP 设置在首次登录后在管理面板中配置，而不是在这里。
...
- **替代**: 点击 "OPEN WEB UI" 或访问 `http://homeassistant.local:8099`

### 测试模式

启用 `test_mode` 以生成虚拟用户，用于测试匹配算法。
在生产环境中禁用它以防止虚假资料。
