...
此应用允许您通过 SSH 或 Web 终端登录到您的 Home Assistant 实例，从而访问您的文件夹，并包括一个命令行工具，用于重启、更新和检查您的实例。

这是 Home Assistant 提供的 [SSH 扩展][hass-ssh] 的增强版本，专注于安全性、可用性、灵活性，并提供通过 Web 界面访问的功能。
...
虽然此应用是经过精心设计和维护的，并考虑了安全性，但在不正确或不经验的手中，它可能会损坏您的系统。
...
- 默认使用 [ZSH][zsh] 作为其 shell。对于初学者来说更容易使用，对于经验更丰富的用户来说则更高级。它甚至预装了 ["Oh My ZSH"][ohmyzsh]，并启用了一些插件。
- 默认包含一组合理的工具：curl、Wget、RSync、GIT、Nmap、Mosquitto 客户端、MariaDB/MySQL 客户端、Awake ("唤醒网络")、Nano、Neovim、tmux 以及许多常用网络工具。
