...
[SearXNG](https://docs.searxng.org/index.html) 是一个免费的互联网元搜索引擎，可聚合多达 247 个搜索服务的成果。用户既不被追踪也不被建立档案。此外，SearXNG 还可以通过 Tor 进行在线匿名搜索。

Home Assistant 插件改编自 https://github.com/DDanii/HA-Add-ons-by-DDanii/tree/master/searxng

该插件包含一个轻量级的 MCP 服务器，为 llama.cpp（以及任何其他 MCP 兼容客户端）提供通过私有 [SearXNG](https://github.com/searxng/searxng) 实例的网页搜索。

MCP 服务器改编自 https://github.com/jdeath/mcp-searxng-enhanced，以提供快速 MCP IP 端点（使用 AI 进行编辑）。对于在 `https://github.com/jdeath/mcp-searxng-enhanced` 上的 MCP 代码。
...
如果启用 set_base_url_for_ingress，它将设置 SEARXNG_BASE_URL 环境变量，这是 ingress 使用所需的，并覆盖 settings.yml 中的 base_url 变量

## 自定义

在插件配置文件夹（addon_configs/2effc9b9_searxng_with_mcp）的第一次运行后，将在其中创建一个 custom.sh 文件，您可以在其中添加自己的命令。
---
