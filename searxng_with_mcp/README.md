# 家居助理插件：searxng 与 mcp

## 关于

[SearXNG](https://docs.searxng.org/index.html) 是一个免费的互联网元搜索引擎，可从多达 247 个搜索服务聚合结果。用户既不被追踪也不被建立档案。此外，SearXNG 还可以通过 Tor 进行使用，以实现在线匿名。
...
MCP 服务器由 https://github.com/jdeath/mcp-searxng-enhanced 调整，以提供快速 MCP IP 端点（使用 AI 进行编辑）。对于包含 MCP 代码的 `https://github.com/jdeath/mcp-searxng-enhanced`。
...
如果启用 set_base_url_for_ingress，则设置 SEARXNG_BASE_URL 环境变量，这对于 ingress 使用是必需的，并覆盖 settings.yml 中的 base_url 变量。
