...
![下载量趋势](https://raw.githubusercontent.com/alexbelgium/hassio-addons/master/readarr/stats.png)

## 关于

---

[Readarr](https://github.com/Readarr/Readarr) 是一个用于 Usenet 和 BitTorrent 用户的电子书集合管理器。它可以监控多个 RSS 源以获取您最喜欢的作者的最新书籍，并与客户端和索引器接口以获取、排序和重命名它们。这是书籍管理和自动化（Sonarr 用于电子书）。

此插件基于以下 Docker 镜像：https://github.com/linuxserver/docker-readarr
...
Webui 可以在 <http://homeassistant:8787/readarr> 或通过 Ingress 通过侧边栏找到。
...
| `CONFIG_LOCATION` | str | `/config` | Readarr 配置存储的路径 |
...
您可以通过创建 `/config/addons_config/readarr_nas.yml` 添加环境变量：
