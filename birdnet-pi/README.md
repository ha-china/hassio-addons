## ⚠️ 开启问题 : [[BirdNET-Pi Docker 独立] 服务无法启动（开启于 2025-06-24）](https://github.com/alexbelgium/hassio-addons/issues/1927) 由 [@sirtakahe](https://github.com/sirtakahe)

# Home assistant 插件：birdnet-pi

## 💖 支持开发

我利用业余时间维护这个和其他 Home Assistant 插件：跟上上游的变化、HA 的变化，并在真实硬件上测试需要大量时间（和一些金钱）。我大约使用我超过 110 个插件中的 5-10 个，因此我安装了测试机器（并购买了一些测试服务，如 VPN），这些机器我自己不使用，用于调试和改进插件。

如果这个插件节省了您的时间或简化了您的设置，我将非常感谢您的支持！

[![给我买咖啡](https://img.shields.io/badge/Buy%20me%20a%20coffee-%23d32f2f?logo=buy-me-a-coffee&style=flat&logoColor=white)](https://www.buymeacoffee.com/alexbelgium)
...
_感谢所有为我仓库加星的人！点击下面的图片为其加星，它将出现在右上角。谢谢！_

[![@alexbelgium/hassio-addons 的星标者仓库罗盘](https://raw.githubusercontent.com/alexbelgium/hassio-addons/master/.github/stars2.svg)](https://github.com/alexbelgium/hassio-addons/stargazers)

![下载趋势](https://raw.githubusercontent.com/alexbelgium/hassio-addons/master/birdnet-pi/stats.png)

## 关于

_注意：若要在没有 HomeAssistant（经典 Docker 容器）的情况下使用，请参阅[这里](https://github.com/alexbelgium/hassio-addons/blob/master/birdnet-pi/README_standalone.md)_
...
- 感谢 https://github.com/gdraheim/docker-systemctl-replacement，提供工作的 Docker 系统
- 使用 HA pulseaudio 服务器
- 使用 HA tmpfs 在 RAM 中存储临时文件，避免磁盘磨损
- 将所有配置文件暴露到 /config，以允许持久化和轻松访问
- 允许修改存储鸟鸣的位置（最好是一个外部硬盘）
- 支持ingress，以允许安全的远程访问，而无需暴露端口
...
安装并首次启动插件
Webui 可以通过两种方式找到：
- 从 HA 中使用 Ingress（无密码，但某些功能无法工作）
- 使用 <http://homeassistant:port> 直接访问，端口为 birdnet.conf 中定义的端口。当要求输入密码时，用户名是 `birdnet`，密码是您可以在 birdnet.con 中定义的密码（默认为空）。这与插件选项中的密码不同，后者是访问 Web 终端的密码

Web 终端访问：用户名 `pi`，密码为插件选项中定义的密码

您需要一个麦克风：可以使用连接到 HA 的麦克风或 RTSP 摄像机的音频流。
...
```yaml
BIRDSONGS_FOLDER: 存储鸟鸣文件的文件夹 # 如果您想避免分析时的磁盘堵塞，应该是一个 SSD
MQTT_DISABLED : 如果为真，将禁用自动 MQTT 发布。仅当本地代理已经可用时才有效
LIVESTREAM_BOOT_ENABLED: 从启动或设置中启动直播流
Use_tphakala_model_v2: false # 切换到 BirdNET-Go 分类器文件
PROCESSED_FOLDER_ENABLED : 如果启用，您需要在 birdnet.conf（或 birdnet 设置）中设置最后保存的 wav 文件数量，这些文件将保存在 tmpfs 内的临时文件夹 "/tmp/Processed" 中（因此不会造成磁盘磨损），如果您想检索它们。这个数量可以从插件选项中调整
TZ: Etc/UTC 指定一个时区使用，请参阅 https://en.wikipedia.org/wiki/List_of_tz_database_time_zones#List
pi_password: 设置访问 Web 终端的用户密码
localdisks: sda1 # 将您的硬盘硬件名称（用逗号分隔）或其标签放入挂载中，例如 sda1, sdb1, MYNAS...
...
选项 1：安装 let's encrypt 插件，生成证书。默认情况下，它们存储在 /ssl 中为 certfile.pem 和 keyfile.pem。只需在插件选项中启用 SSL，它就会工作。
...
根据我的测试，只有使用 KT0210 的转换器（如 Ugreen 的）才能工作。我无法检测到基于 ALC 的转换器
...
结论，使用 Dahua 的麦克风已经足够好，EM272 是最优的，但 Boya by-lm40 是一个非常好的折中方案，因为 birndet 模型分析 0-15000Hz 范围
