...
[jellyfin](https://jellyfin.org/) 组织个人媒体库中的视频、音乐、直播电视和照片，并将其流式传输到智能电视、流媒体盒子和移动设备。这个容器作为一个独立的 jellyfin 媒体服务器进行打包。
...
| `DOCKER_MODS` | list | | 用于硬件加速的额外 Docker 修改 |
...
对于需要 GuC 提交以实现稳定硬件编码的 Intel 系统（例如，N6005），将 `i915_enable_guc` 设置为 `2` 以在容器启动时应用内核参数。此插件仅写入现有的运行时模块参数；不会尝试重新构建内核或更改启动参数。如果主机内核中缺少路径 `/sys/module/i915/parameters/enable_guc` 或为只读，插件将记录警告并继续而不进行修改。
...
> 注意：
> 上述命令创建了一个没有密码的 PFX 文件，您可以使用 `-passout pass:"your-password"` 填写密码，但也必须在 Jellyfin 的配置中提供 `your-password`
...
4. 对于 `自定义 SSL 证书路径:`，指向您的 PFX 文件，并在需要时填写 `证书密码`
5. 滚动到底部并 `保存`
