# 家居助手插件：Vaultwarden (Bitwarden RS)

使用插件 `env_vars` 选项来传递额外的环境变量（大小写均可）。请参阅 https://github.com/alexbelgium/hassio-addons/wiki/Add-Environment-variables-to-your-Addon-2 获取详细信息。

# ⚠️ 建议使用官方插件（https://github.com/hassio-addons/addon-bitwarden）而不是这个分支。这个分支的唯一优点是自动发布，现在官方插件也已实现。⚠️


我在业余时间维护这个和其他的 Home Assistant 插件：跟踪上游更改、HA 更改，并在真实硬件上测试需要花费大量的时间（以及一些金钱）。我经常使用 5-10 个我超过 110 个插件中的几个，所以我安装了测试机器（并购买了一些我自身不使用的测试服务，如 vpn）来调试和改进插件
...
[![GitHub Super-Linter](https://img.shields.io/github/actions/workflow/status/alexbelgium/hassio-addons/weekly-supelinter.yaml?label=代码基础Lint)](https://github.com/alexbelgium/hassio-addons/actions/workflows/weekly-supelinter.yaml)
...
1. 将我的插件仓库添加到您的 Home Assistant 实例中（在监督者插件存储的右上角，或者如果您已配置我的 HA，请点击下面的按钮）。
...
由 Frenck 创建，请参阅此处文档：https://github.com/hassio-addons/addon-bitwarden
