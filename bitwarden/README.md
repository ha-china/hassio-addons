...
# ⚠️ 建议使用官方 add-on (https://github.com/hassio-addons/addon-bitwarden) 而不是这个分支。这个分支的唯一优势是自动发布，现在官方版本也已实现。 ⚠️


我在业余时间维护这个和其他 Home Assistant add-ons：跟上上游的变更、HA 的变更，并在真实硬件上进行测试，这需要很多时间（和一些金钱）。我大约使用我超过 110 个 add-ons 中的 5-10 个，因此我会安装一些测试机器（并购买一些我自身不使用的测试服务，如 VPN）来调试和改进这些 add-ons。
