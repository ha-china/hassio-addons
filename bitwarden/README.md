...
使用 add-on 的 `env_vars` 选项来传递额外的环境变量（名称可以是大小写）。详情请参阅 https://github.com/alexbelgium/hassio-addons/wiki/Add-Environment-variables-to-your-Addon-2。

# ⚠️ 建议使用官方插件（https://github.com/hassio-addons/addon-bitwarden）而不是这个分支。这个分支的唯一优势是自动发布，现在官方插件已经实现了这个功能。 ⚠️


我利用业余时间维护这个和其他 Home Assistant 插件：跟上上游的变化、HA 的变化，并在真实硬件上测试，这需要很多时间（和一些钱）。我大约使用我超过 110 个插件中的 5-10 个，因此我安装了一些测试机器（和一些我本人不使用的测试服务，例如 VPN），以便于调试和改进插件。
