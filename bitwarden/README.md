...
# ⚠️ 建议使用官方插件（https://github.com/hassio-addons/addon-bitwarden）而不是这个分支。这个分支的唯一优点是自动发布，现在已集成到官方版本中。⚠️


我在业余时间维护这个和其他 Home Assistant 插件：跟踪上游变化、HA 变化，并在真实硬件上进行测试需要花费大量时间（和一些金钱）。我经常使用我超过 110 个插件中的 5-10 个，因此我安装了测试机器（并购买了一些我不使用的测试服务，如 VPN），以进行故障排除和改进插件。
