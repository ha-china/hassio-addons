...
使用插件 `env_vars` 选项来传递额外的环境变量（大写或小写名称）。有关详细信息，请参阅 https://github.com/alexbelgium/hassio-addons/wiki/Add-Environment-variables-to-your-Addon-2。

# ⚠️ 建议您使用官方插件（https://github.com/hassio-addons/addon-bitwarden）而不是这个分支。这个分支的唯一优点是自动发布，现在这个功能已经集成到了官方版本中。 ⚠️


我在业余时间维护这个和其他 Home Assistant 插件：跟踪上游更改、Home Assistant 更改以及在真实硬件上进行测试需要花费大量时间（以及一些金钱）。我经常使用我超过 110 个插件中的 5-10 个，因此我会安装测试机器（并购买一些我本人不使用的测试服务，如 VPN）来排查问题和改进插件
...
_感谢每一位给我仓库点星的人！要为它点星，请点击下面的图片，然后它就会显示在右上角。谢谢！_
...
1. 将我的插件仓库添加到您的 Home Assistant 实例中（在管理员的插件存储中右上角，或者如果您已经配置了我的 HA，请点击下面的按钮）。
