...
使用插件的 `env_vars` 选项来传递额外的环境变量（大小写不敏感）。有关详细信息，请参阅 https://github.com/alexbelgium/hassio-addons/wiki/Add-Environment-variables-to-your-Addon-2。

# ⚠️ 建议使用官方插件（https://github.com/hassio-addons/addon-bitwarden）而不是这个分支。这个分支的唯一好处是自动发布，现在已经在官方插件中实现。⚠️


我在业余时间维护这个和其他 Home Assistant 插件：跟进上游更改、HA 更改以及在真实硬件上进行测试需要花费大量时间（以及一些金钱）。我经常使用 5-10 个我的 >110 个插件，所以我安装了测试机器（并购买了一些我不使用的测试服务，如 vpn）来调试和改进插件。
