...
使用扩展的 `env_vars` 选项来传递额外的环境变量（大小写不敏感的名称）。有关详细信息，请参阅 https://github.com/alexbelgium/hassio-addons/wiki/Add-Environment-variables-to-your-Addon-2。

# ⚠️ 建议使用官方扩展（https://github.com/hassio-addons/addon-bitwarden）而不是此分支。此分支的唯一好处是自动发布，现在已实现在官方版本中。 ⚠️


我在业余时间维护这个和其他 Home Assistant 扩展：跟上上游更改、HA 更改以及在实际硬件上进行测试都需要大量的时间（和一些金钱）。我经常使用 5-10 个我 >110 个扩展，所以我安装了测试机器（并购买了些我不使用的测试服务，如 VPN）来排查和改进扩展。
...
1. 将我的扩展仓库添加到您的 Home Assistant 实例中（在管理员控制台扩展商店的右上角，或者如果您已配置我的 HA，请点击下面的按钮）。
