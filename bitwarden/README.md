...
# ⚠️ 建议使用官方插件（https://github.com/hassio-addons/addon-bitwarden）而不是这个分支。这个分支的唯一好处是自动发布，现在这已在官方插件中实现。⚠️


我在业余时间维护这个以及其他一些家居助手插件：跟踪上游变化、家居助手的变化以及在实际硬件上进行测试需要花费大量时间（以及一些金钱）。我经常使用我的 >110 个插件中的 5-10 个，所以我定期安装测试机器（并购买一些我自己不使用的测试服务，如 VPN）来排查问题和改进插件
...
[![GitHub Super-Linter](https://img.shields.io/github/actions/workflow/status/alexbelgium/hassio-addons/weekly-supelinter.yaml?label=代码基线代码审核)](https://github.com/alexbelgium/hassio-addons/actions/workflows/weekly-supelinter.yaml)
...
1. 将我的插件仓库添加到您的家居助手实例中（在 supervisor 插件商店的右上角，或者点击下面的按钮如果您已配置我的 HA）
   [![打开您的家居助手实例并显示添加插件仓库对话框，其中预填充了特定的仓库 URL。](https://my.home-assistant.io/badges/supervisor_add_addon_repository.svg)](https://my.home-assistant.io/redirect/supervisor_add_addon_repository/?repository_url=https%3A%2F%2Fgithub.com%2Falexbelgium%2Fhassio-addons)
1. 安装此插件。
1. 启动插件。
1. 检查插件的日志，查看是否一切顺利。
