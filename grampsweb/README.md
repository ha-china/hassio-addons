# Home Assistant 扩展：Grampsweb

我在业余时间维护这个和其他 Home Assistant 扩展：跟踪上游更改、HA 更改以及在实际硬件上进行测试需要花费很多时间（以及一些金钱）。我经常使用我超过 110 个扩展中的 5-10 个，所以我安装了测试机器（并购买了一些我自己不使用的测试服务，如 VPN），以便进行故障排除和改进扩展。
...
[![GitHub Super-Linter](https://img.shields.io/github/actions/workflow/status/alexbelgium/hassio-addons/weekly-supelinter.yaml?label=代码检查%20基础)](https://github.com/alexbelgium/hassio-addons/actions/workflows/weekly-supelinter.yaml)
...
_感谢每一位给我仓库点星的人！要给它点星，请点击下面的图片，然后它就会显示在右上角。谢谢！_
...
[Gramps Web](https://github.com/gramps-project/gramps-web) 是一个创建和共享家谱的 Web 应用程序。它是免费且开源的家族史软件 Gramps 的 Web 前端。
...
此扩展基于官方 Gramps Web 项目：https://github.com/gramps-project/gramps-web

## 配置

---

Webui 可在 <http://homeassistant:5000> 找到。

### 选项

| 选项 | 类型 | 默认值 | 描述 |
|------|------|---------|-------------|
| `CELERY_NUM_WORKERS` | int | `2` | 后台任务的 Celery 工作者数量 |
| `GUNICORN_NUM_WORKERS` | int | `8` | Web 请求的 Gunicorn 工作者数量 |
...
| `GRAMPSWEB_EMAIL_USE_SSL` | bool | 使用 SSL 加密（用于端口 465） |
| `GRAMPSWEB_EMAIL_USE_STARTTLS` | bool | 使用 STARTTLS 加密（用于端口 587） |
| `GRAMPSWEB_EMAIL_HOST_USER` | str | SMTP 用户名 |
| `GRAMPSWEB_EMAIL_HOST_PASSWORD` | str | SMTP 密码 |
| `GRAMPSWEB_DEFAULT_FROM_EMAIL` | str | 默认的发件人电子邮件地址 |
...
1. 将我的扩展仓库添加到您的 Home Assistant 实例中（在监督器扩展存储的右上角，或点击下面的按钮如果您已配置了 HA）
