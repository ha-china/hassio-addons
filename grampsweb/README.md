...
我在业余时间维护此及其他 Home Assistant 附加组件：跟踪上游更改、Home Assistant 更改以及在真实硬件上测试都需要大量时间（以及一些金钱）。我使用我超过 110 个附加组件中的大约 5-10 个，因此我经常安装测试机（并购买一些测试服务，如 vpn）用于非本人使用的故障排除和改进附加组件。

如果此附加组件为您节省时间或使您的设置更简单，我会非常感激您的支持！
...
_感谢所有人星星我的仓库！如果要星星它，请点击下面的图片，然后它会出现在右上角。谢谢！_
...
[Gramps Web](https://github.com/gramps-project/gramps-web) 是一个用于创建和共享家族树 Web 应用程序。它是 Gramps 的 Web 前端，Gramps 是一款免费开源的家谱软件。
...
Web 界面可在 <http://homeassistant:5000> 找到。

### 选项

| 选项 | 类型 | 默认值 | 描述 |
|------|------|--------|------|
| `CELERY_NUM_WORKERS` | int | `2` | 后台任务的 Celery 工作进程数 |
| `GUNICORN_NUM_WORKERS` | int | `8` | Web 请求的 Gunicorn 工作进程数 |
| `GRAMPSWEB_SECRET_KEY` | str | - | 会话安全密钥（如果未设置则自动生成） |
...
- **env_vars 选项**：使用附加组件的 `env_vars` 选项传递额外的环境变量（名称可以是大写或小写）。详见 https://github.com/alexbelgium/hassio-addons/wiki/Add-Environment-variables-to-your-Addon-2
...
1. 将我的附加组件仓库添加到您的 Home Assistant 实例（在 Supervisor 附加组件商店右上角，或直接点击下方按钮，如果您已配置了此 HA）
   [![打开您的 Home Assistant 实例并显示添加附加组件仓库对话框，预先填充特定的仓库 URL。](https://my.home-assistant.io/badges/supervisor_add_addon_repository.svg)](https://my.home-assistant.io/redirect/supervisor_add_addon_repository/?repository_url=https%3A%2F%2Fgithub.com%2Falexbelgium%2Fhassio-addons)
1. 安装此附加组件。
1. 点击 `Save` 按钮保存您的配置。
1. 将附加组件选项设置为您偏好。
1. 启动附加组件。
...
2. 创建管理员用户账户
3. 设置您的家谱数据库
4. 导入现有的 GEDCOM 文件或开始创建家族树
5. 配置用户权限和共享设置

## 数据存储

附加组件在 `/config` 目录内的几个位置存储数据：
...
- **GUNICORN_NUM_WORKERS**：为增加并发用户而增加
