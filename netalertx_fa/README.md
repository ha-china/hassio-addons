...
我在业余时间维护这个以及其他 Home Assistant 插件：跟上上游变化、HA 变化以及在实际硬件上进行测试都需要花费大量的时间（以及一些金钱）。我经常使用我超过 110 个插件中的 5-10 个，所以我安装了测试机器（并购买了一些我不使用的测试服务，如 VPN），以用于调试和改进插件。
...
[NetAlertX](https://github.com/jokob-sk/NetAlertX) 是一个 WIFI / LAN 扫描仪、入侵检测器和存在探测器，它可以帮助您监控您的网络，以发现新设备和潜在的安全威胁。

**这是全功能访问版本**，相比标准 NetAlertX 插件提供了额外的权限和网络访问功能。
...
Webui 可在 `<您的 IP>:20211` 或通过入口侧边栏访问。
...
此插件支持 MQTT 集成，如果可用，它将自动连接到您的 Home Assistant MQTT 代理。NetAlertX 可以将设备存在信息发布到 MQTT 主题，以便与 Home Assistant 自动化集成。
...
1. 将我的插件仓库添加到您的 Home Assistant 实例中（在管理器插件存储的右上角，或单击下面的按钮如果您已经配置了 HA）
...
如果您需要增强的网络扫描功能，或者标准 NetAlertX 插件没有为您提供足够的网络访问，请使用此版本。

## 支持

在 github 上创建一个问题，或者在 [home assistant 社区论坛](https://community.home-assistant.io/) 上提问。
