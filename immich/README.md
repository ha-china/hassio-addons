...
我在业余时间维护这个和其他 Home Assistant 插件：跟上上游变更、HA 变更以及在真实硬件上进行测试需要花费大量时间（以及一些金钱）。我经常使用我超过 110 个插件中的 5-10 个，所以我安装了测试机器（并购买了一些我自己不使用的测试服务，如 vpn），以便进行故障排除和改进插件。
...
基于网络的文件浏览器。
此插件基于 [docker 镜像](https://github.com/imagegenius/docker-immich)。

## 配置

Web 界面可以在 `<你的 IP>:8080` 找到。PostgreSQL/MySQL 可以是内部或外部的。
