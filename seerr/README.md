...
此扩展打包了 [Seerr](https://seerr.dev/)，一个开源的媒体请求和发现管理器，适用于 Jellyfin、Plex 和 Emby。

此扩展基于现有的 Overseerr 扩展结构，为 Seerr 上游项目和容器镜像进行了适配。它通过内部 NGINX 反向代理支持 Home Assistant Ingress。
...
1. 将我的扩展仓库添加到您的 Home Assistant 实例中（在右上角的监督者扩展存储中，或如果您已配置我的 HA，则点击下面的按钮）。
...
| 选项 | 类型 | 默认值 | 描述 |
|--------|------|---------|-------------|
| `NODE_MEMORY_LIMIT` | int | `512` | Node.js 堆内存的最大值（以 MB 为单位）。如果 Seerr 因为大型库而崩溃，请增加此值；在内存受限的系统上减少此值。 |
...
1. 停止 **Overseerr** 扩展。
2. 安装并启动 **Seerr** 扩展一次，以创建其配置目录（`/addon_configs/db21ed7f_seerr/`），然后停止它。
3. 打开 **[文件浏览器](https://github.com/alexbelgium/hassio-addons/tree/master/filebrowser)** 扩展（或任何具有访问 `/addon_configs/` 的文件管理器）。
