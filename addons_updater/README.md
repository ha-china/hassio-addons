...
这个脚本允许根据上游的新发布自动更新插件。这只是一个帮助工具供开发者使用。最终用户不需要这个来更新他们的插件 - 当有更新可用时，HA会自动提醒他们。
...
在你的仓库的插件文件夹中（位于config.json的位置），创建一个`updater.json`文件。
这个文件将被插件用来获取插件的上游信息。
只有带有updater.json文件的插件会被更新。
这里是一个[示例](https://github.com/alexbelgium/hassio-addons/blob/master/arpspoof/updater.json)。

你可以在文件中添加以下标签：

- github_fulltag: true例如是"v3.0.1-ls67"，false是"3.0.1"
- github_beta: true/false；是否只查找发布版本或预发布版本
- github_havingasset : true如果有要求发布版本有二进制文件而不仅仅是源代码
...
- upstream_version: 自动填充，对应插件中引用的上游版本
