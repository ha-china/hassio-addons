...
⚠️ 该项目正在进行非常活跃的开发。请预期存在 bug 和更改。请勿仅将其作为存储照片和视频的唯一方式！(来自开发者)

我在业余时间维护这个及其他 Home Assistant 附加组件：追踪上游更改、HA 更改以及在真实硬件上测试需要大量时间（以及一些金钱）。我使用约 5-10 个我的 >110 个附加组件，因此我定期安装测试机器（并购买一些我自己不使用的测试服务，如 vpn）来故障排除和改进这些附加组件
...
_谢谢 everyone 给我的仓库星标！要为其星标，点击图片以下，然后它将位于右上角。谢谢！_
...
这是一款直接在您的智能手机上进行的自托管照片和视频备份解决方案，支持 CUDA 硬件加速。这是提供使用 NVIDIA GPU 进行机器学习任务硬件加速的 CUDA 启用的 Immich 变体。

此附加组件基于 imagegenius 的 [docker 镜像](https://github.com/imagegenius/docker-immich)，并启用了 CUDA 支持以增强性能。

## Immich v3

此附加组件追踪 **Immich v3**（基于 `ghcr.io/imagegenius/immich:3-cuda` 镜像行）。

- **数据库**：Immich v3 需要带有 **VectorChord (`vchord`)** 扩展的 PostgreSQL (14–17)；上游 `pgvecto.rs` 支持已被移除。此存储库中的 `Postgres 15` 和 `Postgres 17` 附加组件已提供具有 VectorChord 能力的数据库，并且是推荐的选择（您也可以使用官方的 `ghcr.io/immich-app/postgres:*-vectorchord*` 镜像）。
- **从 Immich v2 升级**：保留您现有的具有 VectorChord 能力的数据库，以便 Immich 可以自动迁移其数据。如果您的数据库仍然包含在旧的 `pgvecto.rs` 扩展中的数据，请保留该扩展，直到 Immich 完成迁移到 VectorChord。
...
- **NVIDIA GPU**：兼容具有 CUDA 支持的 NVIDIA 显卡
- **CUDA 驱动程序**：NVIDIA 驱动程序必须在主机系统上正确安装
- **架构**：仅限 AMD64（ARM 架构不可用）
...
| `JWT_SECRET` | str | | 用于身份认证鉴的 JWT 密钥 |
| `DISABLE_MACHINE_LEARNING` | bool | `false` | 禁用 ML 功能（不推荐用于 CUDA 变体） |
| `MACHINE_LEARNING_WORKERS` | int | `1` | ML 工作者数量（使用 CUDA 时可以增加） |
...
| `skip_permissions_check` | bool | `false` | 跳跳过文件权限检查 |
...
1. 将 `localdisks` 选项设置为驱动器名称（例如，`sda1`）。驱动器将挂载在 `/mnt/sda1`。
2. 将 `data_location` 选项设置在已挂载驱动器上的路径，例如 `/mnt/sda1/immich`。
...
- **env_vars 选项**：使用附加组件 `env_vars` 选项传递额外的环境变量（大小写字母均可）。有关详细信息，请访问 https://github.com/alexbelgium/hassio-addons/wiki/Add-Environment-variables-to-your-Addon-2。

## 安装

此附加组件的安装非常直白，与其他任何 Hass.io 附加组件的安装没有区别。

**先决条件：**
- 具有 CUDA 支持支持的 NVIDIA GPU
- 主机系统上安装了 NVIDIA 驱动程序
- AMD64 架构（ARM 不支持）

**步骤：**
1. 将我的附加组件存储库添加到您的 Home Assistant 实例中（在 supervisor 附加组件商店的右上角，或如果您已配置我的 HA 则点击以下按钮）
...
1. 仔细根据您的偏好配置附加组件，请查看官方文档。

**数据库设置：**
注意：您需要安装单独的 postgres 附加组件以能够连接数据库。您可以安装我存储库中已有的 postgres 附加组件。
注意：在启动之前更改密码；启动后无法更改

## 支持

在 GitHub 上创建 issue，或在 [home assistant thread](https://community.home-assistant.io/t/home-assistant-addon-immich/282108/3) 提问
