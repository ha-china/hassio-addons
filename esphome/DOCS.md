...
5. 点击“打开 Web UI”以打开 ESPHome 仪表板。系统会要求您输入 Home Assistant 的凭证 - ESPHome 使用 Home Assistant 的认证系统来登录您。
...
如果设置为 `true`，这将启用流模式，使 ESPHome 隐藏所有潜在的私人信息。例如 WiFi (B)SSID（这些SSID可能被用来定位您的位置）、用户名等。请注意，您需要在 YAML 文件中使用 `!secret` 标签来阻止这些信息在编辑和验证时显示。