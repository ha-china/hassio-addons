...
该应用允许您编写配置，无需编程经验即可直接将微控制器转化为智能家居设备，只需通过 Home Assistant 即可完成。您只需编写 YAML 配置文件；其余操作（包括空中更新和编译）均由 ESPHome 自动处理。
...
使用 ESPHome，您可以从几行 YAML 代码直接生成自定义固件。例如，要将 [DHT22][dht22] 温湿度传感器包含在内，您只需在配置文件中添加 8 行 YAML 代码：
