...
我利用业余时间维护此插件及其他 Home Assistant 插件：跟进上游变更、适应 HA 更新以及在真实硬件上进行测试非常耗时（也花费了一些资金）。我大约有 5-10 个插件使用频率很高，因此我安装了一些测试机（并购买了一些测试服务，如 vpn），我自己不使用它们来调试和改进这些插件。

如果此插件能为您节省时间或使您的配置更简单，我将不胜感激您的支持！
...
_感谢所有为我仓库点亮了星标的人！要星标它，请点击下图，它将显示在右上角。谢谢！_
...
[Codex](https://github.com/ajslater/codex) 是一个基于网页的漫画归档浏览器和阅读器。
...
1. 将我的插件仓库添加到您的 Home Assistant 实例中（在 supervisor 附加组件商店右上角点击，或者如果您已配置了我的 HA，则点击下方的按钮）
   [![打开您的 Home Assistant 实例并显示附加组件仓库对话框，预填充特定仓库 URL。](https://my.home-assistant.io/badges/supervisor_add_addon_repository.svg)](https://my.home-assistant.io/redirect/supervisor_add_addon_repository/?repository_url=https%3A%2F%2Fgithub.com%2Falexbelgium%2Fhassio-addons)
1. 安装此插件。
1. 点击 `保存` 按钮以保存您的配置。
1. 将插件选项设置为您的偏好设置。
1. 启动插件。
...
| `CODEX_SKIP_INTEGRITY_CHECK` | 跳过启动时数据库完整性修复 | - | `1` |
| `csrf_allowed` | 允许访问应用程序的地址逗号分隔列表 | `http://homeassistant.local:8123,https://homeassistant.local:8123` | `http://localhost:8123` |
...
- **env_vars 选项**：使用插件的 `env_vars` 选项传递额外的环境变量（大写或小写字母名称）。详细 please 参见 https://github.com/alexbelgium/hassio-addons/wiki/Add-Environment-variables-to-your-Addon-2。
