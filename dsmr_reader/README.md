...
[![打开您的 Home Assistant 实例并显示带有预填特定仓库 URL 的添加应用程序仓库对话框](https://my.home-assistant.io/badges/supervisor_add_addon_repository.svg)](https://my.home-assistant.io/redirect/supervisor_add_addon_repository/?repository_url=https%3A%2F%2Fgithub.com%2Fsanderdw%2Fhassio-addons)
[![社区论坛][论坛盾牌]][论坛]

提供一种工具，方便地从您的智能电表通过 DSMR 协议提取、存储和可视化数据。

感谢 Dennis Siemensma 创建了 DSMR 读取器软件（https://dsmr-reader.readthedocs.io）以及 Bram van Dartel 创建了底层的容器镜像（https://github.com/xirixiz/dsmr-reader-docker）。
...
2. 在 TimescaleDB 插件的配置标签中添加 ```dsmrreader``` 数据库作为额外的数据库条目。不需要在 timescale_enabled 下设置它。
3. 启动 TimescaleDB 插件以初始化。
4. 安装此插件。
5. 在 ```配置``` 标签中配置 HA 插件设置。注意：如果您使用此插件作为远程接收器/使用标准 web 服务器或自定义的（如反向代理），您需要通过选择 ```显示禁用端口``` 并将所需的端口号放在那里来打开端口。
6. 启动 DSMR 读取器插件。
7. 在 DSMR 读取器 UI 中转到 ```配置``` 页面（等待应用程序初始化）
8. 使用 admin/admin 登录。
...
注意：需要在命令行中执行命令？在进入容器 bash ("```docker exec -it addon_0826754b_dsmr_reader bash```") 后，您需要执行此命令 "```. /cli-helper.sh```" 以正确应用插件配置标签中的设置。

[dsmr-盾牌]: https://img.shields.io/badge/DSMR%20Reader%20Version-%206.0-purple.svg?style=flat-square
