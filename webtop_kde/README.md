...
默认情况下，镜像基于 abc 用户，我们建议使用此用户，因为所有初始化和配置都是围绕它进行的。默认密码也是 abc。如果你想更改此密码并在访问界面时需要身份验证，只需在 webtop 中的图形终端中输入 passwd。然后，在访问网页界面时使用路径：
...
所有潜在的环境变量请在此处查看：https://docs.linuxserver.io/images/docker-webtop#optional-environment-variables
...
localdisks: sda1 # 将你的驱动器的硬件名称或标签以逗号分隔挂载，例如。 sda1, sdb1, MYNAS...
