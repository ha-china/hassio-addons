# Home Assistant 扩展：Webtop KDE Alpine


我在业余时间维护这个和其他 Home Assistant 扩展：跟进上游更改、Home Assistant 更改以及在真实硬件上进行测试需要花费大量的时间和金钱（以及一些金钱）。我经常使用我 >110 个扩展中的 5-10 个，所以我安装了测试机器（并购买了某些测试服务，如 vpn），这些服务我自己并不使用，以便排查问题和改进扩展

如果这个扩展节省了您的时间或使您的设置更容易，我将非常感激您的支持！
...
这个扩展基于 docker 镜像 https://github.com/linuxserver/docker-webtop
...
默认情况下，镜像基于 abc 用户，我们建议使用此用户，因为所有 init/config 都是基于它的。默认密码也是 abc。如果您想更改此密码并在访问界面时需要身份验证，请简单地在 webtop 中的 gui 终端中执行 passwd 命令。然后，当访问 Web 界面时，请使用以下路径：
...
在此处查看所有潜在的环境变量：https://docs.linuxserver.io/images/docker-webtop#optional-environment-variables
...
此扩展的安装非常简单，与安装任何其他扩展没有不同。

1. 将我的扩展存储库添加到您的 Home Assistant 实例中（在管理器扩展存储库的右上角，或点击下面的按钮如果您已配置我的 HA）
