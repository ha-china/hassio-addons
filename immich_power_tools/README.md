...
我在业余时间维护这个以及其他 Home Assistant 插件：跟进上游更改、Home Assistant 更改以及在真实硬件上测试都需要花费大量时间（以及一些金钱）。我经常使用大约 5-10 个我 >110 个插件，所以我安装了测试机器（并购买了一些我自己不使用的测试服务，如 vpn），以便进行故障排除和改进插件。
...
[Immich Power Tools](https://github.com/varun-raj/immich-power-tools) 为组织和管理您的 Immich 照片库提供了高级工具。此插件通过强大的功能扩展了 Immich 的能力，包括照片组织、分析和管理的功能。

主要功能：
- 高级照片组织工具
- 批量操作照片管理
- 智能分析照片并标记
- 地理位置照片映射，集成 Google Maps
- 重复检测和管理
- 高级搜索和过滤功能
...
| `EXTERNAL_IMMICH_URL` | str | **必需** | 外部 Immich 服务器 URL 用于浏览器访问 |
...
| `GOOGLE_MAPS_API_KEY` | str | | 用于地理位置功能的 Google Maps API 密钥 |
...
1. **运行中的 Immich 服务器** - 此插件需要一个正常工作的 Immich 安装
