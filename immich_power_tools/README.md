...
我在业余时间维护这个和其他 Home assistant 插件：跟踪上游更改、HA 更改以及在真实硬件上测试需要花费很多时间（还有一些金钱）。我经常使用我超过 110 个插件中的 5-10 个，所以我安装了测试机器（并购买了一些我不用自己的测试服务，如 vpn），用于调试和改进插件。
...
[Immich Power Tools](https://github.com/varun-raj/immich-power-tools) 提供了用于组织和管理您的 Immich 照片库的高级工具。此插件通过强大的照片组织、分析和管理功能扩展了 Immich 的功能。

主要功能：
- 高级照片组织工具
- 批量操作用于照片管理
- 基于人工智能的照片分析和标记
- 地理位置照片映射与 Google Maps 集成
- 重复检测和管理
- 先进的搜索和筛选功能
...
| `IMMICH_API_KEY` | str | **必需** | Immich API 密钥用于身份验证 |
...
| `DB_PORT` | str | **必需** | 数据库端口（通常为 `5432` 对于 PostgreSQL） |
| `GOOGLE_MAPS_API_KEY` | str | | Google Maps API 密钥用于地理功能 |
| `GEMINI_API_KEY` | str | | Google Gemini API 密钥用于人工智能功能 |
...
在开始使用此插件之前，请确保您有以下条件：

1. **Immich 服务器正在运行** - 此插件需要一个可用的 Immich 安装
