...
![WordPress Logo](https://raw.githubusercontent.com/FaserF/hassio-addons/master/wordpress/logo.png) 
...
[![Docker 镜像](https://img.shields.io/badge/docker-0.3.1-blue.svg?logo=docker&style=flat-square)](https://github.com/FaserF/hassio-addons/pkgs/container/hassio-addons-wordpress)
...
1. 如果您启用 **SSL** (`ssl: true`)，则对 HTTP 端口的请求将严格重定向到 HTTPS 端口。
2. 确保您的 `wordpress_url` 配置与您打算使用的协议匹配（例如，以 `https://` 开头，如果使用 SSL）。

## 🔐 首次运行 & 登录

### 初始凭证

在 **首次启动** 时，应用将安装 WordPress 并自动生成一个安全的 **管理员密码**。
