...
NIGHTLY 构建是 Music Assistant `dev` 分支的自动构建。它们包括：
...
如果您已经读到这一步，并且仍然想使用 NIGHTLY，那么您可能就是合适的使用者。欢迎来到前沿！ 🚀
